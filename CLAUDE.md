...
**`operation`** is a top-level coordinator row for an OCR-backed target workflow (oath-signature / emergency-contact / onbase / **i9-check's "Run I-9 Check"**), created at PDF-upload time in the target workflow's own panel. The OCR run is delegated under it (`parentRunId`), so the OCR review row stays the one real row in the OCR panel — never duplicated. Before approval the operation row denormalizes the OCR status (`data.ocrStatus`/`data.ocrStep`/`data.ocrRunId`/`data.ocrSessionId`) and renders the status *label* (the other `ocr*` fields back routing + the link, not the display) plus an "Open OCR review" link that switches to the OCR panel, selects the review row, and opens its Preview tab; after approval its fanned-out signer/contact children (stamped `operation-member` and parented to it) render as inline expandable member rows under that coordinator, not a separate operation drill-in queue page. The OCR approve fan-out stamps `operation-member` only when the run belongs to an operation coordinator (`isOperationCoordinatorWorkflow(operationWorkflow)` — oath-signature / emergency-contact / onbase / i9-check); a standalone OCR run keeps the natural archetype. It is a **display** row with no daemon task of its own (stamped explicitly at `/api/ocr/prepare`, NOT a `WorkflowArchetype`/`deriveRowArchetype` output). Oath Upload is deliberately **not** an operation row — it is a real daemon `single` task born at upload (option A) that walks `OCR prep → awaiting approval → wait signatures → submit` as one row; see `src/workflows/oath-upload/CLAUDE.md`.

**Not every coordinator ends at an approval.** A read-only form spec with no approve target may set `OcrFormSpec.completeDelegatedRun` — its DELEGATED run completes terminal `done` right after enrichment instead of parking at `awaiting-approval` (there is nothing to approve). Today that is the **i9-check** workflow's **"Run I-9 Check"** (`i9` spec): on completion `/api/ocr/prepare` enqueues one **REAL `i9-check` member task** per person (`enqueueI9CheckMemberTasks` — the UCPath search + roster EID re-match + master retention-tracker append, run in the dedicated single-UCPath-browser i9-check daemon; the workflow split — 2026-07-17, formerly a separations input-union mode — makes a retry structurally unable to become a termination), and the coordinator stays `running` until the members-terminal rollup completes it. Pages that can never be searched still emit **display-only** failed member rows (`data.displayOnly === "true"`, no daemon task) — `isDisplayOnlyRow` strips retry/cancel/bump and leaves only delete (no task exists to replay; `control/ops/retry.ts` also refuses JSONL reconstruction for any i9-check-flagged row). See `src/workflows/i9-check/CLAUDE.md`.
...
- **Lessons (add vs. audit):** record a new gotcha with the `custom-hr-lesson` skill (it self-dedupes against neighbors before appending). Periodically GC the lesson stores with the `custom-hr-lesson-audit` skill (`.agents/skills/custom-hr-lesson-audit/`) — it sweeps every `LESSONS.md` + `CLAUDE.md` / `AGENTS.md` "Lessons Learned" section to remove duplicate/superseded/dead-reference lessons and verify each remaining lesson still maps to real code. Lessons are maintained, not append-only.
