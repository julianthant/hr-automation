...
**Emergency-contact "Run" button (2026-04-28).** When the operator selects the `emergency-contact` workflow, a primary "Run" CTA appears in the TopBar's queue zone. It opens a PDF upload modal; the dashboard backend OCRs the file via `src/ocr/` (Gemini multi-key + 7-provider fallback), matches each parsed person against the latest roster, and (for unmatched names) enqueues into the eid-lookup daemon to recover EIDs. While that runs, a "preview row" sits at the top of the QueuePanel with a stage-progress strip; once all records reach a terminal state the row flips to "ready for review" — the operator can edit any field inline (persisted to localStorage), select the records to approve, and click Approve to fan out N kernel queue items via the existing daemon path. This is the FIRST in-dashboard launch surface; the runner removal note above still applies to every other workflow. Implementation: `src/ocr/`, `src/workflows/emergency-contact/{prepare,match,roster-loader,preview-schema}.ts`, `src/tracker/{emergency-contact-http,multipart-helper}.ts`, and the `Run*` / `Preview*` components under `src/dashboard/components/`.
