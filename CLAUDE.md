...
- **batch-parent** — anchor row over N peers. Stamped via `data.archetype` only; the legacy `data.mode === "prepare"` resolver fallback was removed 2026-05-19 (`data.mode` is still stamped for prep-row dashboard logic but is no longer consulted by `resolveRowArchetype`).
...
- **2026-05-19: Archetype legacy fallbacks deleted from `resolveRowArchetype`.** `data.archetype` is the only read path now; missing/invalid stamps default to `single` (or `delegate-child` when `parentRunId` is set). Legacy reads of `data.mode === "prepare"`, `data.taskRole`, `data.requestRole`, and the `data.workflow === "ocr"` branch were removed. Old JSONL rows without `data.archetype` (pre-2026-05-17) resolve to the default — acceptable since they're outside the 30-day dashboard lookback. Write sites still use `deriveRowArchetype` from `src/domain/row-archetype.ts`.
