...
**Settings page** (`components/settings/`, navbar gear) — a full-page view exposing tweakable operator constants (operator identity, annual/fiscal dates, screen size, paths, nav/OCR/timeout tuning, feature flags), the desktop-notification toggles, read-only `.env` credential status, and the embedded **Workflow Editor** as its last section. Persists a SPARSE override to `config/settings.json` (gitignored operator state) via `/api/settings`; `src/config.ts` applies it at module load with precedence **env var > settings.json > code default** (empty file = today's behavior). Backend store mirrors the workflow-presentation override pattern (`src/tracker/settings/`, shared type in `src/domain/settings/types.ts`). Architecture: `src/dashboard/CLAUDE.md` (2026-06-29 lesson).
