...
Dupe-protection: the kernel provides no tracker-side idempotency cache or step-cache (both primitives removed 2026-04-23). Workflows that need to avoid double-submits must use live-page probes — e.g. separations' `ucpath-transaction` calls `findExistingTerminationTransaction(page, employeeName, effectiveDate, templateCodes)` (from `src/systems/ucpath/transaction.ts`) to scan the Smart HR Transactions list for a row matching this employee + effective date + UC_VOL_TERM/UC_INVOL_TERM before submitting; oath-signature checks the "no oath signature" sentinel on the loaded profile. Read-only extraction work (e.g. onboarding's CRM `extraction` step) is re-run on every retry — cheap relative to the correctness cost of serving a stale cached extraction when the user fixed bad data between runs.
