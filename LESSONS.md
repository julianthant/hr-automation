...
- **2026-05-17: data.archetype is the write-side row-type contract.** New write sites stamp archetype; `resolveRowArchetype` still keeps read-time fallbacks for legacy JSONL. See `src/workflows/CLAUDE.md` "Row vocabulary" + `src/domain/row-archetype.ts`.
