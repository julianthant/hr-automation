...
- `SSE /events/run-events?workflow=X&id=Y&runId=Z[&date=D]` — stream kernel session events for a specific run (workflow_start, browser_launch, duo_*, auth_*, item_*, cache_hit, etc.) every 500ms. Same delta semantics as `/events/logs`. Includes a pid+time-window fallback for events lacking `runId` (gated by `RUNID_FALLBACK_UNTIL = 2026-04-26` — TODO delete after that date).
...
- **2026-04-19: Session events now carry runId.** `emitSessionEvent` reads from `AsyncLocalStorage` (via `getLogRunId()`) so events emitted during a tracked workflow item are attributable to that run. Pre-feature events without `runId` get pid+time-window fallback in `/events/run-events` until 2026-04-26.
...
- **2026-04-21: runId format coexists.** Two runId formats live side-by-side in `.tracker/*.jsonl` and `sessions.jsonl`. The kernel emits `randomUUID()` — see `runWorkflow` (`src/core/workflow.ts:315`), `runWorkflowBatch` (`src/core/workflow.ts:445`), and `runWorkflowPool` (`src/core/pool.ts:36`), all of which thread the UUID into `withTrackedWorkflow` via `preAssignedRunId`. The legacy `{id}#N` format is emitted by `withTrackedWorkflow` itself (`src/tracker/jsonl.ts:276`) whenever `preAssignedRunId` is absent — i.e. direct/test callers, plus the kernel's single-run `runWorkflow` path which computes a local UUID for the Stepper/Session but does NOT forward it as `preAssignedRunId` when the outer caller didn't supply one (so tracker entries for single-run kernel workflows still get `{id}#N` even though Session events carry the UUID). A SECOND, separate fallback lives in `src/tracker/dashboard.ts` (first use at line 801, also 922/1255/1267/1311/1395/1408): `e.runId || `${e.id}#1`` synthesizes a runId at read time for entries/logs that have none. The `RUNID_FALLBACK_UNTIL = 2026-04-26` pid+time-window fallback in `dashboard.ts:1149` (for session events without `runId`) can be deleted once no emitters without `runId` remain — audit by that date.
