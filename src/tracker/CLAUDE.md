...
- `cleanOldScreenshots(maxAgeDays, dir)` — still available for **manual**/`npm run clean:tracker` use — deletes PNGs in `.screenshots/` whose filename-embedded ms timestamp is older than `maxAgeDays`. **Not** invoked on dashboard startup or `GET /api/preflight` (see 2026-05-16 lesson below).
...
- **Dashboard cadence (JSONL only):** `createDashboardServer` runs a one-time startup prune at **30 days** via `cleanOldTrackerFiles` only. `GET /api/preflight` re-runs the same JSONL prune at most once per **60s** (throttled in `hono/routes/base.ts`). Screenshot pruning was removed from both paths — use **`npm run clean:tracker`** (or future lifecycle-tied cleanup). Pass `{ noClean: true }` or `--no-clean` to skip startup prune.
