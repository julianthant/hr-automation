...
- `SSE /events/run-events?workflow=X&id=Y&runId=Z[&date=D]` — stream kernel session events for a specific run (workflow_start, browser_launch, duo_*, auth_*, item_*, cache_hit, etc.) every 500ms. Same delta semantics as `/events/logs`. Includes a pid+time-window fallback for events lacking `runId` — **permanently on** (not legacy). Batch/pool/daemon workflows call `Session.launch` at batch scope outside per-item `withLogContext`, so auth + browser-launch session events emit without `runId`; the fallback pairs them to items by PID. Cleaner long-term: filter by `workflowInstance` instead of PID.
...
- **2026-04-21: runId format coexists.** Two runId formats live side-by-side in `.tracker/*.jsonl` and `sessions.jsonl`:
  ...
  - **Fallback status (reviewed 2026-04-23):** the pid+time-window fallback in `dashboard.ts` (near line 1323) is **load-bearing**, not legacy. Batch/pool/daemon workflows call `Session.launch` outside per-item `withLogContext`, so their auth events emit with no `runId`. The fallback attaches them to items sharing the same PID. Date gate removed in 2026-04-23 cleanup. Don't delete without replacing with `workflowInstance`-based filtering.
