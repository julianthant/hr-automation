...
- **2026-05-24: State DB handles must be file-sensitive, not just readiness-sensitive.** `isStateDbReady` caches readiness by DB file fingerprint (`dev`, `inode`, size, `mtimeMs`), not forever. `openStateDb` must also compare the current file identity against the cached connection and reopen when `.tracker/state.db` was deleted/recreated under a live dashboard; otherwise `/events` can keep querying a stale SQLite handle while JSONL and the replacement DB contain current rows. Dashboard projection routes and the entries topic resolve the DB handle at request/tick time for this reason.
...
- **2026-05-27: Queue surface classification is shape × scope.** `queue-surfaces.ts` should classify a row as `{ shape, scope }` from `resolveRowArchetype` plus `parentRunId`, not branch on legacy values such as `batch-parent` / `passive-child` / `delegate-child`. Parent runtime policy owns special child presentation: OCR's `flatMemberChildWorkflows` keeps EID Lookup / Active Check dependency rows as flat delegation members.
