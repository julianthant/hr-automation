...
- **2026-04-21: runId format coexists.** Two runId formats live side-by-side in `.tracker/*.jsonl` and `sessions.jsonl`:
  ...
  - **Cleanup gate:** the `RUNID_FALLBACK_UNTIL = 2026-04-26` pid+time-window fallback in `dashboard.ts:1149` (for session events missing `runId`) can be deleted once no emitters without `runId` remain — audit by that date.
