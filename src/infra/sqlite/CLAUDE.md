...
- **2026-06-24: The shim caches compiled statements per connection.** `node:sqlite` (unlike better-sqlite3) does NOT cache prepared statements, so hot paths — the daemon claim loop re-preparing the same claim/countQueued/terminal SQL every iteration — recompiled identical statements on each `db.prepare()`. `wrapDatabase`'s `prepare` now memoizes the `StatementSync` by SQL text for the connection's lifetime (`Map<string, Statement>`, cleared on `close()`). `StatementSync` is safe to reuse across `.run/.get/.all` with fresh params, and `openStateDb` already memoizes the connection per tracker dir, so statements now live as long as it. Callers that bypass the wrapper (constructing `DatabaseSync` directly) get no cache. Verified `close()`-with-cached-statements is safe across the full suite.
