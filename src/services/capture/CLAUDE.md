...
- **Capture requires a public URL; LAN QR fallbacks are disabled — but a missing URL never blocks dashboard startup.** `npm run dashboard` and `npm run dashboard:watch` pass `--capture-ngrok`, which starts `ngrok http <dashboard-port>` and sets `CAPTURE_PUBLIC_URL` to the assigned HTTPS origin before the dashboard binds. An explicitly requested ngrok tunnel that FAILS still exits startup with an error (you asked for a tunnel; you didn't get one). With no tunnel requested and no `CAPTURE_PUBLIC_URL` set (e.g. `dashboard:prod`, or the seeded-fixture verification lane), the dashboard boots with a loud `log.warn` and Capture degrades: `/api/capture/start` returns 503 (`src/services/capture/server.ts`), so no QR ever encodes a LAN URL. Set `CAPTURE_PUBLIC_URL` yourself for a manually forwarded origin. Trailing slash tolerated. Optional static ngrok domains use `CAPTURE_NGROK_URL` / `--capture-ngrok-url`, which are passed to `ngrok http --url`.
...
- **2026-07-02: Phone photo tile stayed BLANK because `photoDisplaySrc` preferred the raw-HEIC object URL over the converted JPEG (`mobile.html`).** Operator screenshots: after taking a photo the grid tile #1 was a blank dark-gray box (the tile background — no image), and the phone lightbox was blank too. Root cause: `handleAddPhoto` sets `previewUrl = URL.createObjectURL(file)` as an instant placeholder, then converts HEIC→JPEG into `dataUrl`. `photoDisplaySrc(p)` returned **`p.previewUrl || p.dataUrl`** — the raw camera file FIRST. An iPhone shoots HEIC, and many browsers (incl. some iOS Safari builds) render a HEIC `blob:` URL as a **blank image, often WITHOUT firing `<img>` onerror** — and because previewUrl was preferred even after the JPEG `dataUrl` landed, the tile never recovered. The comment claiming "iOS Safari decodes HEIC natively" was the wrong assumption. **Fix: prefer the converted JPEG — `photoDisplaySrc` returns `p.dataUrl || p.previewUrl`.** Once conversion lands (heic2any, the same convert already done before upload) the tile shows a guaranteed-renderable JPEG on every browser; previewUrl is now only the placeholder for the sub-second window before `dataUrl` exists. The grid + lightbox `<img>` also keep a small `onerror` that swaps to `dataUrl` if the placeholder can't decode during that window (belt-and-suspenders; the earlier `previewBroken` flag was removed as redundant once dataUrl is preferred). **Verified headless (not blind this time):** booted a fresh `dashboard:prod` (fresh process — note `getCaptureMobileHtml()` CACHES mobile.html at first read, so a running server must be RESTARTED to pick up edits), started a session, drove `/capture/:token` in playwright-cli, and uploaded a real HEIC (`sips -Z 700 "/System/Library/Desktop Pictures/iMac Blue.heic"`) through `#take-btn`→`upload`: the tile `<img>` went from `blob:`/`naturalWidth 0` (old) to `data:image/jpeg`/`naturalWidth 700` rendered (`.screenshots/capture-phone/after-heic-{tile,lightbox}.png`). mobile.html has no build step; inline JS syntax-checked with `node --check`.

- **2026-07-01: Capture ngrok launch path moved onto the standard dashboard command, and LAN fallbacks were disabled.** `npm run dashboard` (and, since 2026-07-02, `dashboard:watch`) passes `--capture-ngrok`, which starts `ngrok http <dashboard-port>` with JSON logs, extracts the `msg:"started tunnel"` `url`, sets `CAPTURE_PUBLIC_URL` in-process before binding the dashboard, and kills ngrok on dashboard exit. Static ngrok domains use `CAPTURE_NGROK_URL` / `--capture-ngrok-url`. If a requested ngrok tunnel fails, dashboard startup exits with an error; a merely *unset* `CAPTURE_PUBLIC_URL` no longer exits (2026-07-02) — startup warns and serves, and `/api/capture/start` returns 503 while it's absent, so no QR ever encodes `http://<LAN-IP>:<port>` (the old hard exit broke `dashboard:prod` and the seeded-fixture verification lane). `CAPTURE_PUBLIC_URL` remains the route contract and `public-scope.ts` still limits that origin to token-gated phone endpoints so a forwarded host cannot expose the whole dashboard.
