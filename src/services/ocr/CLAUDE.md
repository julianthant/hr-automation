...
- **`verify`** — read-only completeness report for a MIXED oath+EC PDF: `rosterMode:"optional"`, `needsLookup → null`, NO approve targets. All enrichment lives in its `enrichRecords` hook (delegates to person-lookup for EID/active/CRM dates and i9-lookup for the I-9 signer; mirrors `src/workflows/ocr/force-research.ts`). Pure helpers `buildVerifyChecks` / `applyPersonLookupToVerifyRecord` / `applyI9ToVerifyRecord` are unit-tested; the live fan-out is not. Those same helpers back the per-record retry: a FAILED check is re-runnable individually from the review pane via `runVerifyRelookup` (`src/workflows/ocr/verify-relookup.ts` → `/api/ocr/verify-relookup`), which re-fans ONE lookup (person or i9) for ONE record and re-patches. Full contract + the orchestrator hooks it relies on: `src/workflows/ocr/CLAUDE.md` (verify + verify-relookup lessons).
