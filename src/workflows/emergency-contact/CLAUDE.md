...
Lives in the sibling [`sharepoint-download/`](../sharepoint-download/) workflow — see its CLAUDE.md for the full story. TL;DR: the Download icon button in every queue-panel header fires `POST /api/sharepoint-download/run` (backed by `buildSharePointRosterDownloadHandler`), which reads `ONBOARDING_ROSTER_URL` from env and saves the xlsx to `src/data/`. The sharepoint-download "workflow" deliberately doesn't register with the kernel, so it stays out of the TopBar dropdown.
...
- **2026-04-22: `sharepoint-download.ts` promoted out of this directory** into `src/workflows/sharepoint-download/` once the dashboard queue-header button made it cross-cutting (every workflow can trigger the download, not just emergency-contact). The pre-flight `runPreflight()` and the dev CLI wrapper both now import `downloadSharePointFile` from that sibling workflow. The new location is intentionally a non-kernel workflow — no `defineWorkflow` call, no tracker JSONL, no dropdown presence — see `src/workflows/sharepoint-download/CLAUDE.md`.
