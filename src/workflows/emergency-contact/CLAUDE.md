...
**Zero-contact employees are the primary working path** (2026-08-05): the editor
search finds the person and presents one blank placeholder contact row, which the
plan fills directly. `NoExistingContactError` still exists but only fires when the
component search itself returns "No matching values were found" (person absent
from the component, not merely contact-less).
...
- **2026-08-05: The contact scroll PAGES one row at a time — drive it in View All with row-anchored ids, and never add-row onto a blank placeholder.** A 31-record live batch failed 100% at Save with "Highlighted fields are required. (15,30)". Three stacked causes: (1) the identity gate's locator anchored on the "Person ID" *label* whose parent contains no EID (PeopleSoft renders label/value as SIBLING divs — fixed via `personIdValue`/`personNameValue`); (2) a zero-contact person already has one blank placeholder row, so the plan's unconditional "Add a new row" created a SECOND blank row that made Save structurally impossible; (3) in the paged scroll, role-based `.first()` selectors interacted with whichever row PeopleSoft rendered after each postback, scattering fills across the two rows. Fix: `prepareTargetContactRow` (View All → blank-row scan → add-row only when every row holds a real contact) + `EMERGENCY_CNTCT_*$row` row-anchored selectors + Contact Name filled LAST with read-back + `assertEmergencyContactPersisted` after Save (positive proof — the previous "no error dialog appeared" check used the never-live-verified `messageDialog` selector and reported a REFUSED save as `done`). Live-probed: a plain `fill()` survives real postbacks in View All mode; a forced `blur()` fires `addchg_win0` and discards the un-committed row — do not add blurs here. A blank row's Relationship dropdown legitimately reads "(Invalid Value)" (default value "O").
