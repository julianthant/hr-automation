...
- `inputSubject`: what the workflow receives — `name | eid | email | kualiId | pdf | selector`, a literal or a resolver `(input) => subject` when it depends on the input variant (person-lookup: `emplId`→eid else name; crm-doc-download: `email`→email else eid). Assignments: name = emergency-contact; eid = work-study/kronos/oath-signature; email = onboarding; kualiId = separations(docId)… (see each workflow); pdf = oath-upload/ocr; selector = sharepoint-download.
...
Internal helper workflow modules may live under `src/workflows/<name>/` without being operator-startable. Pure in-process helpers stay out of `WORKFLOW_LOADERS`; delegated-only daemon workflows belong in `WORKFLOW_LOADERS` so parents can enqueue them, but stay out of dashboard run-surface lists. `src/workflows/person-lookup/` is the operator-facing workflow for both **Search** (Person Organizational Summary by name/EID) and **Match** (HR-Tasks Search/Match by legal name plus SSN/DOB) modes; it is registered in `WORKFLOW_LOADERS` and dashboard input runs, and also exports the `lookupPersonInUcpath` and `handlePersonMatch` primitives for internal callers. `src/workflows/i9-lookup/` is a delegated-only workflow registered in `WORKFLOW_LOADERS`, category `Search`, and absent from input/upload run surfaces. I-9 Check still calls the shared `searchPerson` primitive in-process at its historical `person-match` step; that step label is not a workflow id and must remain stable for tracker rows already on disk.
...
The dashboard's "Edit Data" tab + kernel `prefilledData` channel lets an operator override extracted values and re-run without re-extracting. Opt in by declaring `editable: true` on `detailFields` — the tab is gated on `detailFields.some(f => f.editable)`. **Separations** and **emergency-contact** are opted in today (separations skips extraction steps; emergency-contact overlays corrected EID/contact fields onto the daemon input without re-OCR).

## Lessons Learned

- **2026-07-01: Operator titles show JUST the input — the workflow-name prefix was removed from `buildOperatorSubject` for ALL workflows.** `buildOperatorSubject({ kind, value, prefix })` used to glue the workflow name onto the label (`"Oath Signature EID 00123456"`, `"Separation 4025"`), which then rode `data.__subject` / `__queueTitle` into every queue-row / coordinator / delegation-group title. Operator ask: "the title should just be the input, without the workflow name, for all workflows." The `prefix` argument was deleted (`src/domain/operator-subject.ts`) and stripped from all 11 `operatorSubject:` call sites (crm-doc-download, oath-signature, oath-upload, ocr, emergency-contact, onbase, work-study, separations, old-kronos-reports, sharepoint-download, onboarding). person-lookup / i9-lookup never had one. Identity normalization stays (`displayEid` → `"EID <n>"`, `displayPersonName` → title-cased) — only the WORKFLOW-NAME prefix is gone; the workflow identity now lives only on the rail + the session card (which intentionally shows the label). Pinned by `enqueue-dispatch.test.ts` (separation `__subject` = `"4025"`, not `"Separation 4025"`).
...
- **2026-06-29 / 2026-06-30: `batch` / `batch-member` retired — use `operation` / `operation-member`.** Multi-subject runs emit one coordinator row plus `operation-member` children. Legacy JSONL stamps `batch`/`batch-member` normalize on read via `resolveRowArchetype`. **The 2026-06-30 follow-up completed the rename for ALL leftover operation-surface vocabulary** (identifiers, the `renderAs`/`QueueTitleKind`/`GroupRowVariant` `"batch"` value, the `batch-anchor` scheme, the `alwaysBatch*` policy keys, the `batch-view` action source, and the `*batch-queue*`/`BatchScreenshots`/`BatchFooterActions` dashboard files). `batch` now means ONLY batch *processing* (daemon multi-item run / dependency batches / `batchDisplayOrdinal` / EC YAML input batch) — those are unchanged. The delegation `renderAs` hint value is now `"operation"` (was `"batch"`); it still stamps an `operation-member` child via `rowShape`. See `src/dashboard/components/CLAUDE.md` 2026-06-30 for the full symbol map.
