...
- **2026-05-21: Multi-file uploads are independent single-file runs (was: grouped singles).** The `originBatchRunId` / `originBatchSubject` batch-grouping mechanism was removed from `/api/ocr/prepare`. Each uploaded PDF produces a standalone origin prep row with its own `runId` and no `parentRunId`, rendering as its own queue card titled by PDF name. Selecting N PDFs in the Run modal just fires N separate prepare requests — there is no top-level grouping card anymore.
