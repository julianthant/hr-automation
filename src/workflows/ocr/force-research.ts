...
export async function runForceResearch(input: ForceResearchInput, trackerDirOrOpts?: string | ForceResearchOpts): Promise<void> {
  ...
  // i9 is read-only like verify: after OCR, dedicated i9-check member tasks run
  // the HR-Tasks person search and retention-tracker append. It does not use the
  // oath/EC name-lookup pipeline force-research replays. Re-running it here
  // would clear identity fields and corrupt the report — re-upload/re-OCR is
  // the i9 re-run path.
  if (formType === "i9") {
    throw new Error(
