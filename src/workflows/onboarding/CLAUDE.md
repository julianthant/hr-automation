...
- `retry.ts` — `retryStep(name, fn, opts)` helper: linear backoff, per-attempt error logs, throws `RetryStepError` after exhausting attempts. Used only by `workflow-legacy.ts` (parallel path has no `ctx`) and the dry-run branch in `workflow.ts`. Kernel-side callsites use `ctx.retry` instead
