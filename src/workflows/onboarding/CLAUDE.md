...
The fix lives in this workflow (not the kernel): the `transaction` step now probes the UCPath Smart HR list for an existing in-flight / saved HIRE before re-submitting, via `findExistingHireTransaction` (`src/systems/ucpath/ss-smart-hr.ts`). It runs **after the dry-run guard, before `buildTransactionPlan(...).execute()`**. If a hire is already there it logs, stamps `status: "Already Submitted"` + `existingHireTxn`, screenshots `onboarding-existing-hire-transaction`, and RETURNS without submitting. Mirrors separations' `findExistingTerminationTransaction`.

- **Keyed by NAME, not EID.** A brand-new hire has no Empl ID (the Person ID column renders "NEW" until the transaction processes), so the probe searches the **SS Smart HR Transactions** page by the person's name (`buildHireSearchName` → `"Last,First"`) and matches a `HIR`/`REH` action via the pure `pickHireRow`. Onboarding reaches the transaction step ONLY for a person NOT found in UCPath (rehires short-circuit earlier), so any HIR/REH row under this exact name is overwhelmingly this workflow's own prior submission.
- **Best-effort / fail-open.** A probe navigation/parse failure (or an empty name) degrades to "no existing hire → proceed with submit" — a transient probe glitch never BLOCKS a legitimate hire.
- **The results grid exposes no effdt/template column**, so the match is name + hire-action; `effectiveDate` + `TEMPLATE_ID` are carried into the audit log only. **NEEDS LIVE VERIFICATION:** that the SS Smart HR "Name" search returns a pending hire and that the `Last,First` key is the right format for that box.
