...
- **2026-06-07: No-data / error paths now capture screenshots for operator evidence.** Every early-return and error path in `searching` and `crossVerificationStep` that previously exited without a screenshot now does a best-effort `ctx.captureAndStampScreenshot(...)` before returning or rethrowing. Added captures: (1) EID "Not found" → `personOrgSearchScreenshot` label `"no UCPath detail"` (UCPath grid, ucpath system); (2) name-search ERROR before rethrow → `personOrgSearchScreenshot` label `"UCPath search failed"` (ucpath); (3) `stampCrmStartDateAndScreenshot` no-records path → `crmSearchScreenshot` label `"no CRM record"` (crm); (4) EID CRM search ERROR → `crmSearchScreenshot` label `"CRM search failed"` (crm); (5) name-search CRM ERROR → `crmSearchScreenshot` label `"CRM search failed"` (crm). Each capture is wrapped in its own try/catch so a screenshot failure can never mask the lookup outcome or the original error — on the rethrow path the screenshot always runs BEFORE the throw. **Requires live verification** — screens can only be confirmed with real UCPath/CRM sessions.
