...
- **2026-06-02: The oath operation brands `ou`; the oath-upload ticket row DISPLAYS the OCR root's `ou-…` trace id (root trace-id propagation).** The whole oath operation is branded `ou` via the OCR form spec's `traceCode` (the operator uploads "to Oath Upload", so the trace id names the destination, not OCR's own `oc`). approve.ts reads the OCR root row's frozen `ou-…` id and stamps it as `rootTraceId` on the oath-upload ticket's `__runtimeOptions`; the daemon worker then displays that exact id on every ticket row while the row keeps its own runId/itemId (logs/SQLite/footer `#run` unchanged). DISPLAY-only — oath-upload still WAITS on the signer rows as before (siblings, not delegation). Mechanism in `src/core/CLAUDE.md` ("Root trace-id propagation") + `src/workflows/ocr/CLAUDE.md`. Pinned by `ocr-approve-oath-fanout.test.ts` + `tests/scenarios/delegate-to/oath-upload-shape.test.ts`.
