...
**Idempotency window (crash between ServiceNow POST and ticket persist):**
before a REAL submit the handler stamps `data.submitAttempted: "true"` via
`ctx.updateData` — the very next emit is the `running step=submit` row that
`ctx.step` writes at step START, so the marker is durable on disk BEFORE the
ServiceNow POST. On retry, after the ticket probe misses, the handler calls
`hasUnverifiedPriorSubmit(sessionId, pdfHash, trackerDir)`: a prior marker with
no recorded ticket means the submit outcome is UNKNOWN (the ticket may already
exist), so the handler stamps `status:"submit-unverified"` and THROWS instead
of auto-refiling — the operator verifies support.ucsd.edu manually (record the
ticket if it exists, else re-upload). Dry runs never stamp the marker and skip
the guard (they file nothing). A recorded ticket always wins: the ticket probe
runs first and skips the submit steps.
...
- **2026-07-09: Ticket idempotency window closed + wait-signatures poll failures are distinguishable from signer failures.** Two walk-hardening fixes: (1) a crash AFTER the ServiceNow POST but BEFORE the ticket number persisted used to let a Contract-2 retry re-file a duplicate HR ticket (`findPriorTicketForSession` found no ticket, so the retry submitted again). The handler now stamps a durable `data.submitAttempted` marker on the `running step=submit` row (emitted at step start, before the POST) and refuses to auto-submit when a prior marker exists with no recorded ticket — `hasUnverifiedPriorSubmit`, throws `submit-unverified`, operator verifies ServiceNow manually. (2) `wait-signatures` used to let a `watchChildRuns` rejection (poll error / timeout / blocked parent) escape as a raw watcher error indistinguishable from a signer failure; it now rethrows a distinct `could not verify … signer row(s) … NOT filing` error (CancelledError passes through untouched) so the terminal row says the signer set could not be VERIFIED, not that a signer failed. Pinned by `handler.test.ts` (unverified-submit throw, recorded-ticket precedence, marker-before-step ordering, dry-run exemptions, watch-rejection wrap) + `handler-idempotency.test.ts` (`hasUnverifiedPriorSubmit`).
