...
- **2026-05-18: Oath-signature prep no longer emits a request member row.** The default Oath title is now `Oath · <runIdSuffix>` without `#`. Single-file Oath upload renders the file row with the PDF name as title and the default Oath title as subtitle/footer; there is no synthetic `Oath Signature Request` child because the main prep row already represents the OCR handoff in the queue panel. Multi-file upload is N independent single-file runs (see the 2026-05-21 lesson below): the modal submits N separate `/api/ocr/prepare` requests, each producing its own standalone prep row. OCR-to-EID fan-out stays as delegation member rows, not a batch delegated row. Final oath-signature work rows are person rows titled by the person name when known, falling back to EID.
- **2026-05-21: Multi-file upload no longer groups into one batch card.** The `originBatchRunId` / `originBatchSubject` grouping was removed. Selecting N PDFs in the Run modal now fires N independent `/api/ocr/prepare` requests with no shared parent id, so each PDF is its own top-level prep row/card titled by its PDF name — identical to uploading them one at a time. Removed: `buildBatchSubject` in `RunModal.tsx`, the `originBatchRunId`/`originBatchSubject` fields on `PrepareInput`, and the `/api/ocr/prepare` multipart parse of them. Operator request: each PDF should be treated as a separate single run, not one task with multiple PDFs.
