...
- **2026-06-02: PDF-branch fan-out step renamed `fan-out` → `delegate-signatures`.** The PDF branch's step list is `["ocr", "delegate-signatures", "ucpath-auth", "transaction"]`; the signer fan-out runs inside `ctx.step("delegate-signatures", …)` and `runSignerBranch` skips it. Reuses the codebase's canonical "delegate" vocab. The full operator-visible flow now reads **oath signature → ocr → person lookup → ocr (awaiting-approval) → delegate-signatures (signer data-entry rows inside the batch)**. Scenario `_beats.ts` (`skipStep`/`step` names) and the `cancel-during-*` tests reference the new name; no snapshot churn (step name isn't snapshotted). Don't reintroduce `fan-out`.
- **2026-06-01: UCPath Duo is deferred to the signer branch, not session launch.** The kernel authenticates every declared `system` eagerly at launch (`Session.launch(wf.config.systems)` in-process, and the daemon's startup `authenticating` phase), so a real `ucpath` `login` fired Duo immediately — before OCR/approval — even on the PDF branch (which never touches UCPath) and on oath-upload's delegated child. Fix: `ucpath` `login` is now a no-op, and `runSignerBranch` runs a real `loginToUCPath` inside a `ucpath-auth` step (mirrors oath-upload's ServiceNow deferral). Result: a PDF run never Duos; delegated fan-out signer children Duo only after OCR approval, when they reach UCPath. `loginToUCPath` is idempotent (`"already_logged_in"`), so a daemon Duos once on the first signer item and reuses the warm session for the rest (incl. across `betweenItems: ["reset"]`, which keeps cookies). This also fixes the oath-upload complaint — its own ServiceNow auth was already deferred; the early Duo came from this child.
