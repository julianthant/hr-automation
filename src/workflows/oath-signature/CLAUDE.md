...
- **2026-05-27: Oath-upload starts oath-signature through delegation.** The upload workflow should hand one `{ kind: "pdf" }` child to `oathSignatureWorkflow` and then wait; the oath-signature PDF branch owns OCR, approval, and signer fan-out. Because that PDF run has an oath-upload parent, its row is delegated work, not a natural top-level `batch-parent`.
