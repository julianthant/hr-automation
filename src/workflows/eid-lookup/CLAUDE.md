...
- **2026-05-20: Runtime policy uses default utility row actions; OCR children title by person/EID.** `EID_LOOKUP_WORKFLOW_RUNTIME_POLICY` spreads `DEFAULT_WORKFLOW_RUNTIME_POLICY` and sets `memberRow.titleSource: "person"` so delegation-member rows under OCR/Oath prep prefer `searchName` / resolved EID over technical retry ids. Flat OCR fan-out is still enforced by `OCR_WORKFLOW_RUNTIME_POLICY.delegation.utilityChildWorkflows` — not this workflow's policy.
