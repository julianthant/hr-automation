...
Given one or more Kuali document IDs, for each doc: launch 3 tiled browsers (Kuali, New Kronos, UCPath); **`authChain: "parallel-staggered"`** — every SSO form is filled in parallel, then submits are spaced (`staggerMs`, default 5s) so up to **three Duo prompts overlap** on the phone (approve in any order); extract separation data from Kuali; run a 3-way parallel fetch (New Kronos timecard, **UCPath Workforce Job Summary — now identity-aware: returns found/not-found + the detail-page name**, Kuali timekeeper name fill) via `ctx.parallel`; **conditionally verify identity (the `identity-check` step) — see "Conditional name↔EID verification" below: person-lookup runs ONLY when the Job Summary flags a problem, otherwise the step is skipped (dynamic timeline);** reconcile dates (see "Date model" below); create the UCPath termination transaction; write the transaction ID back to Kuali and save.

## Conditional name↔EID verification (2026-06-18 — READ THIS)

The `identity-check` step (now AFTER `kronos-search`, gated on the Workforce Job Summary result) decides whether to delegate to **person-lookup by NAME**. It runs the lookup ONLY when the Job Summary search is suspicious — it is **not** an every-run pass (that earlier same-day "always delegate" model was replaced). The Job Summary search by the Kuali EID is the gate:

| Job Summary outcome | identity-check |
|---|---|
| **Found** + detail-page name **matches** the Kuali name (`scoreNameMatch ≥ 0.85`) | **SKIPPED** — EID trusted, no person-lookup |
| **Found** + name **mismatch** (the valid-format-but-wrong-person Perez `10694136` case) | **RUNS** → delegate by name → take the name-derived EID |
...
- **Dynamic timeline:** the no-lookup cases call `ctx.skipStep("identity-check")` → the step renders **skipped** and no delegated person-lookup child row appears. When it runs, the delegated row surfaces under the separation. "Used vs not used" is visible for free.
- **Name match uses fuzzy `scoreNameMatch` (≥ 0.85), not exact** — Job Summary renders "First Last", Kuali "Last, First", and middle-name/variant drift is common; an exact compare would fire person-lookup constantly. Lives in `separationNameMatches` (`workflow.ts`); the name selector is `jobSummary.personName` (`#DERIVED_NAME_DISPLAY_NAME`).
- **EID correction → re-fetch:** when person-lookup corrects the EID, the handler **re-fetches the Job Summary** (so dept/payroll is the verified person's — a miss now fails loud) and **re-searches New Kronos** (best-effort — empty timecard falls back to the Kuali LDW). Both use the shared `getJobSummaryIdentity` / `runNewKronosTimecard`.
- The name is authoritative on a delegation; an unresolvable name fails loud (never proceeds unverified). This is the operator-confirmed exception to `src/systems/ucpath/CLAUDE.md` "No cross-source auto-fallbacks" — now CONDITIONAL + VISIBLE, not always-on. Pinned by `resolve-eid.test.ts` + `dry-run.test.ts` (handler wiring) + `steps.test.ts` (order).
...
- **2026-06-18: identity-check is now CONDITIONAL, gated on the Workforce Job Summary result (replaces the same-day "ALWAYS delegate" model — see "Conditional name↔EID verification" above for the full gate table).** The earlier version of this step delegated to person-lookup by NAME on EVERY run, up front (between `kuali-extraction` and `kronos-search`). That was replaced the same day: the EID-format check catches nothing, but the **Workforce Job Summary search by the Kuali EID** is a cheap, reliable gate, so we use it. `identity-check` moved to **AFTER `kronos-search`** (the Job Summary is fetched inside that parallel block, now via the non-throwing identity-aware `getJobSummaryIdentity` → `{ found, name, data }`), and `resolveSeparationEid(ctx, kualiData, { found, name })` is called ONLY when the gate flags a problem: **found + name mismatch** (the Perez `10694136` case) or **not-found + short (<8 digit) EID** → delegate by name, name wins; **not-found + complete 8-digit EID** → FAIL LOUD (no lookup); **found + name match** (`separationNameMatches`, fuzzy `scoreNameMatch ≥ 0.85`) → `ctx.skipStep("identity-check")`, no lookup. So the **common path no longer pays a person-lookup auth pass** — only suspicious runs do. The step's skip/run state is the **dynamic-timeline** signal (skipped = trusted; ran + delegated child row = looked up). On an EID correction the handler **re-fetches** Job Summary (critical — dept/payroll must be the verified person's; a miss fails loud) and **re-searches** New Kronos (best-effort) via the shared `getJobSummaryIdentity` / `runNewKronosTimecard`. Step order is `kuali-extraction → kronos-search → identity-check → ucpath-job-summary → ucpath-transaction → kuali-finalization` (pinned by `steps.test.ts`). Still skipped wholesale on txn-prefilled or kronos-skipped (edit-resume / "Transactions only") paths — no Job Summary gate there. The kuali-extraction schema (`eid: ^\d{5,}$`) stays LENIENT (a short EID must reach the gate, not throw at parse). Pinned by `resolve-eid.test.ts` (`separationNameMatches` + the 5 conditional cases) + `dry-run.test.ts` (handler wiring: name-match skip, mismatch-correct+re-fetch, unverifiable fail, txn-prefilled skip) + `job-summary-propagation.test.ts` (identity shape).
