...
- **2026-04-24: Pre-submit existence check matches by EID, not name.** `findExistingTerminationTransaction(page, employeeId, effectiveDate)` scans the Smart HR Transactions list for a row whose Person ID cell equals `employeeId` AND whose row text includes the effective date (MM/DD/YYYY) AND whose row text includes "Terminat" (covers "Terminatn" display variant). The original signature (`employeeName, effectiveDate, templateCodes`) failed loudly on 2026-04-24: Aki Uchida (EID 10794813) had `"Akitsugu, Uchida"` in Kuali but `"Aki Uchida"` in UCPath — the workflow's `Last, First` → `First Last` transform produced `"Uchida Akitsugu"`, and since "Aki" is a diminutive of "Akitsugu" (5 chars off, not 2), even the added `fuzzyNameMatch` helper couldn't bridge the gap. Worse, the template-code pre-filter (`UC_VOL_TERM` / `UC_INVOL_TERM`) rejected every row before name matching even ran — those template strings aren't in the list row's `textContent` (the visible columns show "JOB" under Type and "Terminatn" under Action). Result: 2 dupe terminations created for the same EID/date. EID match is deterministic; name match is not. The name form, fuzzy helper (`src/utils/fuzzy-match.ts`), and template-code arg are deleted. `readLatestTransactionNumber` was rewritten to the same EID-based row lookup so post-submit txn# readback no longer fails on name mismatch either.
- **2026-04-23: Removed step-cache + idempotency primitives in favor of pre-submit existence check.** `src/core/step-cache.ts` + `src/core/idempotency.ts` + the `runSeparationRecover` / `separation:recover` CLI / `findEmplIdForDoc` helper were all deleted. Dupe-protection for `ucpath-transaction` is now live-page — see the 2026-04-24 entry above for the current EID-based match. On a match, it opens the row's detail page, extracts the txn #, calls `ctx.updateData({ transactionNumber })`, skips the submit, and continues to `kuali-finalization`. On no match, the normal submit flow runs. This replaces: the old `hashKey({ workflow, docId, effectiveDate, ... })` + `hasRecentlySucceeded` / `recordSuccess` cache, the idempotency-readback recovery path on the `:recover` CLI, and the kuali-extraction step-cache (which had served stale wrong-EID data on the doc-3924 retry incident). Kuali extractions are now always re-scrape. Correctness win: the Smart HR list is the source of truth, and a retry against a partially-submitted txn now converges on the real submitted value instead of a cache-recorded echo.
- **2026-04-23: TXN # must be updated inline at every success point in `ucpath-transaction`, not just at handler exit.** The handler's final `ctx.updateData` at the end of the body only fires if every subsequent step returns without throwing. When `kuali-finalization` threw (Kuali save button unreachable, page timeout), the tracker's detail panel showed `Txn # = "—"` even though UCPath had accepted the submit. Fix: explicit `ctx.updateData({ transactionNumber })` at each success site — the pre-submit existence-check branch (where `findExistingTerminationTransaction` returns a matched txn) and the fresh-submit branch (after `readLatestTransactionNumber`) — so the dashboard reflects reality regardless of what happens in kuali-finalization. Design spec: `docs/superpowers/specs/2026-04-23-daemon-isolation-and-separations-stability-design.md` Part 1.1.
- **2026-04-23: Wrong EID in Kuali → fail, don't auto-correct.** Earlier in the day we briefly shipped a three-tier cascade (Workforce → Person Org Summary → name-based lookup) that would auto-correct EID typos. User reverted same day: silent auto-correction hides the upstream data problem and risks targeting the wrong employee when names aren't unique. The canonical behavior is now: `getJobSummaryData` throws with a clear "verify EID in Kuali Build" message, separations' step fails, user corrects Kuali, re-runs. The pre-submit existence check (`findExistingTerminationTransaction`) prevents duplicate submits across the retry — no tracker-side cache needed.
...
- **2026-04-22 / 2026-05-04: `runSeparationCli` pre-emits `pending` at enqueue time; SQLite is now queue authority.** Old flow waited for the daemon's first tracker row before the dashboard Queue panel would populate — which meant 4 Duo approvals worth of blank UI even though 10 doc IDs had been typed. Current flow: the CLI adapter passes `onPreEmitPending` into `ensureDaemonsAndEnqueue`, which calls it immediately after inserting each SQLite task row and appending the JSONL enqueue audit event (one `pending` row per docId, same `{docId}` data shape as `runSeparationBatch`). `enqueueItems` pre-assigns a UUID v4 as the `runId`; `claimNextItem` claims the matching SQLite task transactionally so the daemon's downstream `running`/`done` rows share the same runId as the pre-emitted pending row (no duplicate rows in the dashboard). The same pattern is wired into `runWorkStudyCli`, `runEidLookupCli`, and `runOnboardingCli`.
...
- **2026-04-22 (same day, later): `ensureDaemonsAndEnqueue` now enqueues BEFORE awaiting spawn.** Old order was `spawn → enqueue → wake`; if the user ran `npm run separation 3927 3924 ...` against no alive daemon, the 10 doc IDs sat unwritten while the CLI blocked up to 5 minutes on Duo. If the CLI timed out (e.g. the pid bug above) the items were lost entirely. Current order is `validate → findAliveDaemons → enqueueItems (with pre-assigned runId) → onPreEmitPending callback → wake alive daemons → spawnDaemon (serial) → await`. Items land in SQLite `tasks` in <10ms (with JSONL audit beside them), the dashboard sees pending rows immediately, and newly-spawned daemons self-claim from the already-populated queue once their auth chain completes. Spawn is still awaited at the end so the CLI only exits after every requested daemon is registered (gives callers a chance to see spawn failures).
- **2026-04-22 / 2026-05-04: Daemon mode (`runSeparationCli`) with SQLite queue authority.** `npm run separation <id...>` enqueues to an alive daemon (or spawns one) instead of launching 4 fresh browsers + re-Duo-ing every invocation. Multi-daemon dispatch uses shared SQLite `tasks` rows claimed in one transaction: start two daemons with `npm run separation 123 -p 2`, and the next batch of three docs enqueues to the shared queue — whichever daemon finishes its current doc first grabs the next queued row. JSONL queue files are audit-only; **SQLite is the queue authority**. `npm run separation` exposes `-n` / `-p` only (see `src/cli.ts`). Kernel behavior per-doc is byte-identical because the daemon calls `runOneItem` in a loop — only `withBatchLifecycle` + session-drawer aggregation differ at scope boundaries. **Design doc**: `docs/superpowers/specs/2026-04-22-workflow-daemon-mode-design.md`.
- **2026-04-21: Batch-level instance for sequential-batch runs.** `runWorkflowBatch` sequential branch now runs inside `withBatchLifecycle` (`src/core/batch-lifecycle.ts`). One `workflow_start` / `workflow_end` per batch — the session drawer shows ONE `Separation N` row for a batch of N docs instead of N. One `Session.launch` + one `SessionObserver` captures `authTimings` for all 4 systems (**parallel-staggered** launch in current code); those timings are injected into every item's tracker rows as synthetic pre-handler `auth:<id>` entries with real start timestamps. The `betweenItems: ["reset"]` hook still runs between items inside the lifecycle body. SIGINT fans out `failed` rows for un-terminated items + emits one `workflow_end(failed)`.
- **2026-04-17: Migrated to the kernel.** `runSeparation` + `runSeparationBatch` are CLI adapters over `runWorkflow` / `runWorkflowBatch`. `authChain: "parallel-staggered"` replaces older hand-rolled auth orchestration — see `workflow.ts` module comment for staggered Duo behavior. `ctx.page(id)` awaits each system's ready promise, so `ctx.parallel({ oldK, newK, jobSummary, kualiTimekeeper })` inside Phase 1 blocks on the correct Duo per task. Batch mode via `runWorkflowBatch` sequential — browsers reused across docs, `session.reset(id)` runs between docs for all 4 systems (each has a `resetUrl`). The kernel self-registers dashboard metadata via `defineWorkflow` (nothing extra in `index.ts`). Don't reintroduce raw `launchBrowser` / `withTrackedWorkflow` / `withLogContext` / `ensurePageHealthy` calls in the workflow or CLI adapters — those live in `src/core/` now.
...
- **2026-04-10: Batch mode design** — For processing multiple separations, launching + authenticating 4 browsers per doc ID was too slow. Fix: batch mode launches browsers once, authenticates once, processes each sequentially reusing the same browser sessions. The kernel's `runWorkflowBatch` sequential mode now does this declaratively via `batch: { mode: "sequential", betweenItems: ["reset"] }`.
...
- **2026-04-10: Parallel-staggered auth + work via ready promises** — Work can start on each browser as soon as **that** browser's Duo completes, while other Duos may still be pending. The declarative shape is `authChain: "parallel-staggered"` on `defineWorkflow` (`workflow.ts`).
- **2026-04-20: Per-system auth steps + screenshot framework + txn # readback.** Kernel now auto-prepends `auth:<id>` step names and emits step events per system via `makeAuthObserver`. Added form screenshots at UCPath-submit and Kuali-save sites via `ctx.screenshot({ kind: "form", label })`. `fillTransactionResults` now uses `fillWithVerify` (readback + `type()` retry). Edit-data-capable `detailFields` expanded in `workflow.ts` (`lastDayWorked`, `comments`, etc.).
- **2026-05-15: Settled-result handling is shared.** Phase-1 parallel work still intentionally uses `ctx.parallel` and `PromiseSettledResult` so Kronos/Kuali non-fatal branches can be classified without failing the whole step. Reuse `settled.ts` (`logSettledRejection`, `unwrapSettled`) for that classifier/log/throw pattern instead of repeating four status checks in `workflow.ts`.
