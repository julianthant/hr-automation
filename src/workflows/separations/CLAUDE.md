...
- `workflow.ts` — Kernel definition (`separationsWorkflow`) + CLI adapters (`runSeparation`, `runSeparationBatch`, `runSeparationCli`). `runSeparationCli` is the daemon-mode entry used by `npm run separation` — validates via `SeparationInputSchema` and forwards to `ensureDaemonsAndEnqueue(separationsWorkflow, docIds.map(docId => ({docId})), { new, parallel })`. `runSeparation` / `runSeparationBatch` are in-process paths for tests/scripts.
...
- **2026-04-22 / 2026-05-04: Daemon mode (`runSeparationCli`) with SQLite queue authority.** `npm run separation <id...>` now enqueues to an alive daemon (or spawns one) instead of launching 4 fresh browsers + re-Duo-ing every invocation. Multi-daemon dispatch uses shared SQLite `tasks` rows claimed in one transaction: start two daemons with `npm run separation 123 -p 2`, and the next batch of three docs enqueues to the shared queue — whichever daemon finishes its current doc first grabs the next queued row. JSONL queue files remain audit/history and `HRAUTO_QUEUE_BACKEND=jsonl` fallback. `--direct` flag retained for tests and edge cases. Dry-run still bypasses the daemon entirely. Kernel behavior per-doc is byte-identical to the legacy path because the daemon just calls `runOneItem` in a loop — only the instance wrapping (`withBatchLifecycle`) + session-drawer aggregation differ. **Design doc**: `docs/superpowers/specs/2026-04-22-workflow-daemon-mode-design.md`.
