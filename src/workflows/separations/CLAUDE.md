...
| Field | Value | Why |
|-------|-------|-----|
| `archetype` | `"single"` | A dashboard separation enqueue is one Kuali doc and should render as one flat row. The `batch` config below is session reuse, not row shape. |
