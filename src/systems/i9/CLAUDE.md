...
- Remote Section 1 creation is confirmed in this order: exact live success alert (`Remote Access Form I-9 email(s)|message(s) has been sent`) or `.ErrorMessage` with error precedence; only then click OK; then require the same `/employee/profile/{profileId}` route. A hidden Create button is never proof of success.
...
- **2026-07-16: Observe the Remote I-9 callback before dismissing it, and never retry an uncertain create.** The live production `employee.profile.js` callback posts to `/form-remoteaccess/create-newhire`, emits one of two exact success alerts, binds failures to `.ErrorMessage`, and redirects to the same profile after alert OK. The old flow clicked a generic OK first, then searched for a broad phrase or accepted a hidden Create button; onboarding also retried the mutation without repeating its SSN search. `createI9Employee` now gates on the exact alert/error and identity-bound route, while onboarding performs one create attempt per search-first run.
