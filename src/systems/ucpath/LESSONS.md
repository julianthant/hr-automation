...
## 2026-08-20 — Smart HR Save and Submit can land on the "Person Match Found" review page, which the outcome poll never watched

**Tried:** `clickSaveAndSubmit` clicked `Save and Submit` (control id `HR_TBH_WRK_TBH_SAVE$4$`, proven by the action-bar diagnostic), then polled `waitForTransactionOutcome(errorBanner, confirmationOkButton)` for the definitive outcome, exactly as it had for every hire filed on 2026-08-18/20.

**Failed because:** For two hires (Hao Sun 99d5012c, Emily Robles ebd5d59e — three attempts between them) UCPath rendered neither the error banner nor the confirmation OK: it rendered PeopleSoft's own Search/Match review page **"Person Match Found"** — a "Possible Person Matches" grid (Person ID / Name Type / Name Effective Date / Legal First Name / Legal Last Name / masked National ID / masked DOB / Gender / Per Org Summary) with a `Select` button per row and `Not a Match - Continue with Hire` / `Save for Later` / `Cancel` underneath. The poll had no signal for it, so the submit "timed out with no error banner and no confirmation OK dialog" — correct in that it refused to report success, but blind: the log could not say WHY, and the hire silently never filed (the page persists nothing; Transactions in Progress stayed empty). The failure screenshots (`.tracker/screenshots/onboarding-<email>-error-transaction-ucpath-17872471{49489,88470}.png`) were the only evidence. Candidates were a namesake with a different SSN + DOB (Robles) and ten same-surname people (Sun), none of them the hire.

**Fix:** `clickSaveAndSubmit` polls THREE named signals — `classifySubmitSignals(errorVisible, personMatchVisible, okVisible)`: error banner first, then `smartHR.personMatchFoundHeading` (`getByText("Person Match Found", { exact: true })`), then the confirmation OK (checked LAST because `getByRole("button", { name: "OK" })` is generic). On the review page it reads the grid by HEADER text (`readPersonMatchCandidates` via `smartHR.personMatchCandidateRows` — rows carrying a `Select` button; throws if the header row is missing, never "no candidates") and decides with the pure `decidePersonMatchContinue`: click `smartHR.personMatchNotAMatchButton` ("Not a Match - Continue with Hire") ONLY when every candidate is excluded by a hard identifier known on BOTH sides that differs (DOB month/day — UCPath masks the year — or SSN last-4; a name is never evidence) or by an operator-reviewed EID (`notMatchEids`, onboarding's `data.notMatchEids` prefilled channel). Anything else returns `{ success:false, error }` naming the unresolved candidates and the exact `prefilledData.notMatchEids` to re-run with. Selectors were mapped from the live failure screenshots (role/name on rendered text) and are exercised by the next submit that hits the page — a drift fails loud, never silent.

**Tags:** smart-hr, save-and-submit, person-match-found, search-match, possible-person-matches, not-a-match, wrong-person, outcome-poll, timeout, onboarding, notMatchEids
**References:** `src/systems/ucpath/transaction.ts` (`classifySubmitSignals`, `decidePersonMatchContinue`, `readPersonMatchCandidates`), `tests/unit/systems/ucpath/transaction.test.ts` (live candidate tables pinned), `src/workflows/onboarding/CLAUDE.md` (2026-08-20 section). Companion to `#2026-08-06` (DOB-keyed fuzzy Search/Match — the same engine, one step earlier).

## 2026-08-20 — SS Smart HR "Name" box matches the DISPLAY name (`First Last`), and a single match auto-opens the Transaction Details page

**Tried:** `findExistingHireTransaction` / `readSubmittedHireReceipt` searched the SS Smart HR Transactions Name box with `buildHireSearchName` = `Last,First` (e.g. `ALNASSER,ALI`), expecting a results grid to scan (`scanSsSmartHrResults`), exactly as designed 2026-07-01.

**Failed because:** Live probes (`ALNASSER,ALI`, `Alnasser,Ali`, `Alnasser`, `ALNASSER`, `Salazar,Lenny`, `Salazar`) all returned "No matching values were found", while `Ali Alnasser` / `Lenny Salazar` / `Jaden Campos` each returned **1 of 1** and `Hao Sun` returned **1-2 of 2** — including the unprocessed hire `T002216675 … HIR Pending`. So the probe had ALWAYS fallen open (name key wrong), and the belief that "the SS Smart HR list does not carry an unprocessed hire" (handoff 2026-08-20 §5.2, `readSubmittedHireReceipt` demoted to a fallback) was an artefact of that key. Second surprise: when exactly ONE transaction matches, PeopleSoft's Find-an-Existing-Value skips the grid and opens that transaction's detail page directly (Transaction Status / Transaction ID spans, a "Hire Details" grid with `HIR`, and the routing strip `HIRE Expand Transaction: T…, ID: …, Effdt: 2026-09-11, …`), so a grid scan there sees nothing.

**Fix:** `buildHireSearchName` → `First Last`. New `readSsSmartHrSearchOutcome` classifies the post-search page: `detail` (the `#UC_SS_TRANSACT_UC_TRANSACT_ID` span resolves exactly once — read txn/status spans, `detailPageEffdt` (`Effdt: YYYY-MM-DD`) and `detailPageHireAction` (`HIR`/`REH`) off the body text), `grid` (existing scan + drill-in), or `none`. Both the pre-submit duplicate probe and the receipt readback consume it. Live-verified 2026-08-20 against real names: Hao Sun (grid, Pending → found/skip at matching effdt, not found at a different effdt), Ali Alnasser (detail, Approved), Emily Robles (detail, Pending), unknown name → none; receipts read back `T002216675 Pending` / `T002216676 Pending`.

**Tags:** ss-smart-hr, name-search, first-last, auto-drill, transaction-details, duplicate-hire, receipt, onboarding, live-verified
**References:** `src/systems/ucpath/ss-smart-hr.ts` (`buildHireSearchName`, `readSsSmartHrSearchOutcome`, `detailPageEffdt`, `detailPageHireAction`), `tests/unit/systems/ucpath/ss-smart-hr.test.ts`, `src/workflows/onboarding/CLAUDE.md` (Retry safety). Supersedes the "NEEDS LIVE VERIFICATION" caveats from `#2026-07-01` in those docs.

## 2026-08-20 — Workforce Job Summary "Effective Date" must be zipped from the frozen LEFT table; the right row's date is "Expected Job End Date"

**Tried:** `extractJobInfo` scanned every `<tr>`, kept rows whose first cell was a 6-digit Job Code, and took the row's first `MM/DD/YYYY` cell as its Effective Date (pass 1), with a count-exact "Effective Date"-header zip as pass 2 only when NO row carried an in-row date (per the 2026-06-24 lesson). `extractWorkLocation` did the same anchored on the Position Number cell (`cells[p-2]`, else first date cell).

**Failed because:** The Job Summary grids are PeopleSoft frozen-column grids: the LEFT data table `tdgblWF_JOB_SUMM$0` holds Org Rel · Empl Rec · **Effective Date** · Seq and the row-aligned RIGHT table `tdgbrWF_JOB_SUMM$0` holds the tab's columns — on Job Information its ONLY date column is **Expected Job End Date**. Pass 1 therefore "found" a date in every right row (the END date: 06/30/2025 · 09/21/2025 · 06/30/2026 · 09/20/2026 for student appointments), pass 2 never ran, and `pickEffectiveDatedRow` saw every STDT 3 row as eff 06/30/2026 (AFTER the 06/16/2026 separation) and fell back to a 09/21/2025 STDT 2 row. Live 2026-08-20, separations docs 4540 (Kim, Jon 10791600 — promoted to STDT 3 eff 03/29/2026) and 4541 (Zermeno, Leselie 10769919 — STDT 3 from 09/15/2025): Kuali Payroll Title Code/Title were filled 004921 / STDT 2 instead of 004920 / STDT 3. Work Location's `querySelectorAll("tr")` also matched the OUTER container rows (logged 5–6 junk rows: a row with the EID as "position", stray dates, `<none>`) and was only right because every row was dept 000412.

**Fix:** `readFrozenGridDump` dumps the `tdgbl*`/`tdgbr*` data tables (content document = `#main_target_win0` iframe when present, else the page); pure `pairFrozenGridRows` pairs them by grid-id suffix, requires equal row counts and exactly ONE all-dates column in the left table (otherwise THROWS — a misaligned zip or a moved column must never pick a row), and `jobInfoRowsFromGrid` / `workLocationRowsFromGrid` project the right-row cells (Job Code(0)/Desc(1); Position anchor → Dept ID +3 / Dept Desc +4, throwing on a missing anchor). Both extractors poll the scan (`pollForGridScan`; `pollForJobInfoScan` kept as the job-info specialisation) with `node:timers/promises` sleeps. Unit-pinned with dumps transcribed from BOTH live grids (`tests/unit/systems/ucpath/job-summary.test.ts`): 06/16/2026 → 004920 STDT 3 (eff 06/12/2026); 03/01/2026 → STDT 2; mismatched counts / no date column / non-job-code first cell all throw. Never read a date out of the right-table row — the frozen left table is the only source of a row's Effective Date.

**Tags:** job-summary, workforce-job-summary, frozen-column, tdgbl, tdgbr, effective-date, expected-job-end-date, job-information, work-location, payroll-title, stdt-3, separations, kuali, wrong-row, live-verified
**References:** `src/systems/ucpath/job-summary.ts` (`readFrozenGridDump`, `pairFrozenGridRows`, `jobInfoRowsFromGrid`, `workLocationRowsFromGrid`, `pollForGridScan`), `tests/unit/systems/ucpath/job-summary.test.ts`, `src/workflows/separations/CLAUDE.md` (Job Summary date-correctness note), `src/systems/ucpath/CLAUDE.md` (2026-08-20). Supersedes the in-row-date wording of `#2026-06-24` (the effective-date RULE there still stands; only the date SOURCE changed).

## 2026-08-21 — UC_CONC_HIRE (concurrent hire on an existing Empl ID): the "Enter Transaction Details" page resolves a NAME for ANY EID, Continue raises a "Person ID already exists" dialog, and the form lands on Job Data with only 3 tabs

**Tried:** Reusing the UC_FULL_HIRE plan shape (reason code → Personal Data landing tab → Job Data → Earns Dist → Employee Experience → re-click Personal Data) for the rehire/concurrent-hire template, and treating the Empl ID fill on "Enter Transaction Details" like a plain text field.

**Failed because:** Live map 2026-08-21 (EID 10875503): (1) after the Empl ID blur PeopleSoft resolves the person's name into `#PERSON_NAME_NAME_DISPLAY` — and a bogus `10000001` resolved to a REAL different employee, so the fill alone proves nothing about WHO the transaction targets; (2) Continue (after the Reason Code) raises a modal `Person ID 10875503 already exists in the system for Alan Yanez Herrera. (1007,61) Select OK to continue the hire process with this Person ID. Select Cancel to enter a new Person ID.` that must be acknowledged via `#ICOK` before the form renders (`selectReasonCode`'s "dropdown gone" post-condition still passes, so nothing upstream notices); (3) the form's tabs are **Job Data (landing) → Earns Dist → Personal Data** — no Employee Experience tab, Personal Data is pre-filled (legal name only, no SSN/DOB/address/Tracker Profile ID fields), and Save and Submit enables once all three are visited; Comments + Initiator Comments live on Job Data. The Job Data grid itself is identical to UC_FULL_HIRE (`fillJobData` worked unchanged; comp-rate ids `$11`). Cancel on the form left NO "Transactions in Progress" row.

**Fix:** `fillTransactionDetailsEmplId` fills the EID, waits for `smartHR.transactionDetailsPersonName` to resolve and RETURNS the name (throws on blank) — the caller (`buildConcurrentHirePlan`) refuses a `different`-tier name vs the CRM person BEFORE the reason code. `acknowledgePersonIdExistsDialog` acknowledges ONLY a dialog matching `already exists in the system` + this EID (no dialog / any other dialog throws). `waitForJobDataForm` is the post-Continue named condition; the tab walk is Job Data → Earns Dist → `clickPersonalDataTab` + `readPersonalDataLegalName` (second wrong-person readback). Dry run asserts Save and Submit ENABLED then `cancelTransactionDraft` (`smartHR.cancelTransactionButton`). Template id `UC_CONC_HIRE` ("Staff Concurrent Hire/Inter Location Transfer"), reason `Concurrent Hire - Non Dual Emp`, comments `Concurrent Hire as Dining Student Effective <effdt>. PCN <position>. Job number #<recruitment>.` (`buildConcurrentHireCommentsText`). Other EID templates seen in the lookup: `UC_CONC_HIRE_AC`, `UC_REHIRE` (UC Rehire - Staff Only), `UC_REHIRE_AC`, `UC_REHIRE_REI`, `UC_REHIRE_REI_AC`.
