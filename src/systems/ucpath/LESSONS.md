...
## 2026-07-01 — Person-search National Id magnifier raises a MAIN-PAGE modal mask that blocks the Search click (skip the magnifier; wait for Search enabled)
...
**Fix:** Do NOT click the magnifier at all (for any record). Live-verified 2026-07-01 (dummy NID 123456789 + Test/Person + 01/01/2000): filling the criteria alone flips the Search button `DERIVED_HCR_SM_SM_SEARCH_BTN` from `PSPUSHBUTTONDISABLED` to `PSPUSHBUTTON` (enabled) — each field fill's FieldChange server roundtrip enables it — and a click then advances straight to the "Search Results" grid (order "10 NID Only"). `searchPerson` now `await waitForPersonSearchButtonEnabled(page, frame)` (bounded poll on `btn.isEnabled()`) before the Search `safeClick`; the magnifier click + `ensureNoBlockingModal` are removed. NB: the dialog's OK button IS `id="#ICOK"` (`onclick="closeMsg(this)"`, class `PSPUSHBUTTONTBOK`) and `document.getElementById("#ICOK")` on the MAIN frame DOES find + clear it (mask → `display:none`) — the dismiss selector was correct; the real fault was the wrong-frame mask wait plus clicking the unnecessary magnifier at all. The `personSearchCriteriaSufficient` fail-fast guard (neither SSN nor DOB → throw) is kept.
...
**References:** `tests/unit/systems/ucpath/navigate.test.ts`; live-verified via `playwright-cli` 2026-07-01 (`.screenshots/ucpath-person-search/search-results-after-fix.png`). **NEEDS LIVE VERIFY:** a real onboarding record end-to-end (only replicated on the dummy NID here).
