...
**Confirmed exception — separations `identity-check` (2026-06-18, now CONDITIONAL).** Separations DOES name-correct the EID, but only when the **Workforce Job Summary search flags a problem** — it is NOT an every-run pass (an earlier same-day version delegated always; that was replaced). The gate: Job Summary FOUND + detail-page name mismatches Kuali (the `10694136`-for-the-wrong-person case) OR Job Summary NOT found + a short (<8 digit) EID → delegate to person-lookup BY NAME (name wins, fails loud if unverifiable); Job Summary NOT found + a complete 8-digit EID → FAIL LOUD with no lookup; Job Summary FOUND + name matches → trust the EID, no lookup. This reads the name via the new identity-aware `getJobSummaryIdentity` (`job-summary.ts`, returns `{ found, name, data }`, non-throwing on a miss); `getJobSummaryData` is now a thin throw-on-miss wrapper over it. It is NOT the hidden in-read cascade reverted on 2026-04-23 — it is a VISIBLE, conditional pipeline step (`identity-check`, AFTER `kronos-search`) + a delegated person-lookup child row that appears only when a lookup actually runs. The Workforce-read no-fallback rule below is unchanged (`getJobSummaryIdentity`/`getJobSummaryData` still do no cross-source fallback themselves); the name-based correction lives only in the separations handler. See `src/workflows/separations/CLAUDE.md` ("Conditional name↔EID verification").
