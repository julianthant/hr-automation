...
- **2026-07-08 (live-verified 2026-07-16): `switchToPreviousPayPeriod` asserts the displayed period actually changed, not just that the dropdown closed.** The link-hidden wait only proves the click registered. The registry selector `#timeframe-selector-input` is read before/after and a blank or unchanged value throws. Live verification observed `Current Pay Period → Previous Pay Period` and the grid changed from 7/05–7/18 to 6/21–7/04; the method uses the registry locator for its DOM click.
