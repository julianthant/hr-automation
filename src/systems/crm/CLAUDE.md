...
- Hardcoded column indices in search results: "Offer Sent On" is column index 1; Process Stage is index 4
- `selectLatestResult` skips the header-ish "Search Results" row (no name link) and dead stages (`Offer Rescinded` / cancelled / withdrawn). Picking the newest date on a rescinded re-offer opens a record with no iDocs viewer (`PDF.js did not load`). Live 2026-09-16: Hector Fuentes had Sept 14 Rescinded + Sept 10 Campus Forms Pending — the pending record is the one with documents.
- Date parsing is lenient (`new Date(dateText)`). If every *live* linked row has an unparseable date, throws: "CRM returned search rows but no parsable Offer Sent On date". A header-only scrape (results not yet painted) throws "no record links" instead of that date error.
...
- `idocs-download.ts` exports `downloadCrmIdocsDocuments`, `DEFAULT_CRM_DOC_INDICES`, and `resolveDefaultCrmDocIndices` — a direct-fetch / PDF.js-bypass path for downloading onboarding PDFs from the CRM iDocs viewer (`/iDocsForSalesforce/…`). `resolveDefaultCrmDocIndices` only includes Doc 3 (index 2, EE Data Gathering Form) when `totalDocs` is 9; for 5-doc records (or anything other than 9) it skips the form and downloads Doc 1 (Signed Offer Letter) only.
