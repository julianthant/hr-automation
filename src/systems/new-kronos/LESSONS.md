...
## 2026-06-22 — Date-range inputs: reject `fill()`, race a fixed-delay multi-key type — drive them ONE settled keystroke at a time (OBS-006 / ISS-B05)

**Tried:** (1) `safeFill(timecard.startDateInput(page), startDate, ...)`. (2) Real keystrokes of the full formatted string `pressSequentially("05/10/2026", { delay: 20 })`. (3) Digits-only at a fixed delay + readback-verify + retry: `press("ControlOrMeta+a"); press("Delete"); pressSequentially("05112026", { delay: 60 })` then compare `dateDigits(inputValue())`.
**Failed because:** The WFD date-range `<input type=text>` controls are **JS-masked, React-controlled, and async**: (1) `fill()` silently reverts to today's date — no error, wrong range. (2) They auto-insert the `/` separators themselves, so typing the literal slashes races that insertion and scrambles the value (`05/10/2026` → `6/05/1020`). (3) **Digits-only did NOT fix it — ANY multi-key type at a FIXED inter-key delay still races the controlled re-render.** On the live **8-worker parallel** separations batch (24 tiled/backgrounded windows → throttled timers + CDP contention), the mask commits each keystroke slower than 60ms, so the next digit lands mid-render. Deterministic tracker evidence (doc #4292): `wanted 05112026, got 1120260605` (attempt 1) then `1120260622` (attempts 2–3) — **10 digits**, the year segment overflowed to 6 and interleaved today's `0622`. Two tells prove it is a RACE not a clear/format bug: the readback is **non-deterministic** across attempts, and it **overflows** past 8 digits. The round-2 readback-verify correctly turned silent garbage into a loud `WFP-00889` abort, but the dates still never landed (every attempt raced).
**Fix:** Drive the mask **condition-based, never on a guessed delay** — `typeMaskedDate(loc, dateStr, label)` in `navigate.ts` (no `page` arg): per attempt (up to 4×) `click` → `clearMaskedInput` (select-all+Delete, then Backspace until `dateDigits(inputValue())===""`, verified by readback — a single Delete is not trusted) → `typeMaskedDigits`: type the 8 digits ONE at a time via `maskedDigitPrefixes(want)`, and after EACH keystroke poll `waitForMaskedDigits` until the field's stripped digits equal the running prefix (`"0"→"05"→"051"→…→"05112026"`) before sending the next key; bail the attempt the instant a digit fails to settle OR the field **overflows** (more digits than wanted — the mask can only have scrambled, no point waiting). Final `dateDigits` readback must equal `want` or it retries / throws loud. The next key is never sent until the mask has committed the current one, so **there is no race to lose**. Don't "simplify" this back to `pressSequentially`/a fixed delay — that is exactly what failed. `maskedDigitPrefixes` + `dateDigits` are pure and pinned by `tests/unit/systems/new-kronos/navigate.test.ts` (incl. the `1120260622` live-log scramble matching NO prefix). **NEEDS LIVE RE-VERIFY** on a real WFD timecard under the parallel batch — the condition-based contract is confirmed against unit logic only, not the live masked input.
**Selector:** `timecard.startDateInput`, `timecard.endDateInput` in `selectors.ts`
**Tags:** date-range, fill, input, keystrokes, pressSequentially, fixed-delay, race, condition-based, settle, per-keystroke, mask, scramble, overflow, readback, verify, WFP-00889, OBS-006, ISS-B05, setDateRange, timecard, parallel-batch
