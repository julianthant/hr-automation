...
## 2026-07-30 — Date-range fields are dual-mode; calendar selection is the shared safe path (corrects 2026-06-22, OBS-006 / ISS-B05)

**Tried:** First treating the WFD "Select range" Start/End fields as masked text inputs and typing `MM/DD/YYYY`; then, after a 2026-06-22 DOM capture showed native date inputs, treating them as always-native fields and verifying every result with exact ISO string equality.
**Failed because:** Both single-mode models were incomplete. WFD declares `ng-attr-type="{{rangeInput.useNativeDateInput ? 'date' : 'text'}}"`, so the same ids resolve to native `date` fields in some renders and Angular-managed `text` fields in others. Native mode accepts ISO through `fill()` and reads ISO back; text mode wipes programmatic fills on the next digest and reads locale `M/D/YYYY` (with inconsistent zero-padding). On 2026-07-30 the calendar had already selected the correct date, but the old ISO string comparison rejected the text-mode `5/10/2026` readback as different from `2026-05-10`, aborting with `WFP-00889`. Separately, the active-period trigger is icon-only; reading its `textContent()` always produced a blank verification baseline.
**Fix:** `rangeInputMode` reads the resolved `type`. Native mode gets the ISO `fill()` fast path; text/unknown mode goes directly through `pickRangeDateViaCalendar`, the one widget-owned path that works in both modes. `normalizeRangeDateReadback` parses either ISO or locale spelling, rejects impossible/unparseable dates, and `rangeDateSettlesTo` polls the Angular digest before deciding. Each field is verified, the start is re-verified after setting the end, and Apply fails loud before a wrong window can reach the grid. Month steps wait for the calendar header to change rather than sleeping a fixed guess. Period-switch proof reads `timecard.payPeriodLabel` (`#_timeFrame` / `.selected-timeframe`), not the icon-only trigger. Pure helpers and the production regression are pinned in `tests/unit/systems/new-kronos/navigate.test.ts`; the selector catalog was regenerated after live verification on 2026-07-30.
...
**Tags:** date-range, dual-mode, native-input, text-input, angular, iso, fill, calendar, moment-picker, gridcell, month-nav, readback, verify, pay-period-label, WFP-00889, OBS-006, ISS-B05, setDateRange, timecard
...
## 2026-07-31 — "Timecard option never rendered" for every employee after the first was NOT a render race — the daemon never reset the page, so a timecard was ALREADY open

**Tried:** The 2026-06-24 condition-based-waiting fix above (15s `waitFor({state:"visible"})` + one dropdown re-open + anchored `^timecards?$` role-name). It closed the fixed-`waitForTimeout` race correctly, but its "NEEDS LIVE RE-VERIFY under a real multi-doc daemon batch" was never done — and the identical first-works-rest-fail pattern came back.
**Failed because:** A DIFFERENT root cause with the same signature. `session.reset` in the daemon claim loop (`src/core/daemon/daemon.ts`) was gated on `isCancelOutcome`, so it fired only after a CANCELLED item — the daemon never returned New Kronos to `NEW_KRONOS_URL` between docs. Doc #1 starts at `/wfd/home` (fresh auth) and its Go To menu offers **Timecard**, so it navigates fine; from then on the app sits ON a timecard, and the Dayforce Go To menu does not offer Timecard when a timecard is already open — so the anchored option genuinely never renders and no amount of waiting finds it. Live (separations-2026-07-31, 23 docs, one daemon session): exactly 1 `NAV-OK` (EID 10605166, the first) and 20 `NAV-FAIL`, deterministic — a render race would not fail 20/20 while succeeding 1/1. The audit screenshots are the proof: the Employee Search panel shows the CORRECT target selected (`Results for "10598755"`) while the grid behind it still shows doc #1's employee, **Bazan, Fernando (10605166)**. Distinguishing signal vs. a true render race: a race is intermittent and timing-sensitive; this is perfectly deterministic and correlates with batch POSITION, not timing.
**Fix:** Kernel-side, not selector-side — the daemon now resets every system after EVERY completed item (`!state.shuttingDown`), matching the in-process batch/pool paths that always did. `clickGoToTimecard` and the anchored `timecardItem` selector are UNCHANGED and still correct. The fail-loud chain also held perfectly throughout: `clickGoToTimecard` returned false → `runNewKronosTimecard` threw → the settled `ctx.parallel` block logged it and fell back to the Kuali dates, so **no employee's punches were ever attributed to another person** (every failed doc logged `lastPunch=none`). When you see this symptom, check whether the page was reset between items BEFORE re-mapping the dropdown selector.
**Selector:** `clickGoToTimecard` / `goToMenu.timecardItem` (both unchanged); the actual fix is `session.reset` in `src/core/daemon/daemon.ts`
**Tags:** go-to, timecard, dropdown, batch, first-works-rest-fail, page-state-leak, session-reset, daemon, between-items, not-a-render-race, navigation
**References:** `src/core/CLAUDE.md` 2026-07-31; `src/workflows/separations/CLAUDE.md` 2026-07-31
