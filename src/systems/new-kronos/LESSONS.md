...
## 2026-06-22 — Date-range inputs: reject `fill()`, AND scramble when fed the literal `/` — type digits only + verify (OBS-006 / ISS-B05)

**Tried:** (1) `safeFill(timecard.startDateInput(page), startDate, ...)`. (2) After that failed, real keystrokes of the full formatted string: `await loc.press("ControlOrMeta+a"); await loc.pressSequentially("05/10/2026", { delay: 20 });`.
**Failed because:** (1) The WFD date-range `<input type=text>` controls silently revert to today's date on `fill()` — no error, wrong range loaded. (2) These inputs are **JS-masked and auto-insert the `/` separators themselves**, so typing the literal slashes from the string RACES that auto-insertion and intermittently **scrambles the digits**: on the live separations batch 2026-06-22 a correct `05/10/2026 – 07/10/2026` (confirmed in the tracker log entering `setDateRange`) landed in the field as `6/05/1020 - 6/07/1020`, which WFD rejected with **`WFP-00889 The date is outside of the valid range of dates`**. The SAME code had been "verified live 2026-06-18" for `4/01/2026 - 5/05/2026` — so it's a timing race (works sometimes), not a regression, and the result was applied **silently** (no readback) so garbage reached WFD.
**Fix:** `typeMaskedDate(page, loc, dateStr, label)` in `navigate.ts`: type **digits only** (the `dateDigits` helper strips/zero-pads to `MMDDYYYY` so the mask supplies the separators), then **read `loc.inputValue()` back, normalize via `dateDigits`, and compare** — retry up to 3× and **throw a clear error** instead of applying a wrong range. `dateDigits` pads each component so it matches WFD's non-padded-month readback (`6/05/2026` ≡ `06/05/2026`). Pinned by `tests/unit/systems/new-kronos/navigate.test.ts` (`dateDigits`). **NEEDS LIVE RE-VERIFY** on a real WFD timecard — the digit-typing + readback contract was confirmed only against unit logic, not the live masked input.
**Selector:** `timecard.startDateInput`, `timecard.endDateInput` in `selectors.ts`
**Tags:** date-range, fill, input, keystrokes, pressSequentially, mask, scramble, readback, verify, WFP-00889, OBS-006, ISS-B05, setDateRange, timecard
