...
- **2026-06-16: Kuali form inputs return whitespace-padded values** — A live read-only run (doc #4241) extracted an EID of `" 10772489"` (leading space) and Location `"Catering "` (trailing space). The padding broke `SeparationDataSchema`'s `eid: ^\d{5,}$` and risked the vol/invol exact-match on `terminationType`. Fix: `extractSeparationData` now `.trim()`s every extracted field (`employeeName`, `eid`, `lastDayWorked`, `separationDate`, `terminationType`, `location`). This is normalization of the same value — NOT a cross-source correction. A wrong Kuali EID must still fail loudly (no Workforce/name-search fallback). See `LESSONS.md`.
