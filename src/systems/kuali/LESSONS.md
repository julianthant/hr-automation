...
## 2026-06-24 — Department dropdown match must be bidirectional (UCPath descriptions carry a division prefix the Kuali label lacks)

**Tried:** `pickDepartmentOptionIndex` matched only when the UCPath dept description was a SUBSTRING of a Kuali option.
**Failed because:** UCPath "VCSA-CL ASSOCIATED STUDENTS" / "VCSA CAMPUS RECREATION" never matched Kuali "… Associated Students" / "… Campus Recreation" (the UCPath description carries a division prefix the Kuali label lacks) → "No matching department found" → department left blank.
...
**Selector:** `finalTransactions.department` in `selectors.ts` (consumed by `fillFinalTransactions` in `navigate.ts`)
**Tags:** select, selectOption, option, department, bidirectional, division-prefix, tier-2, final-transactions, separation, kuali

## 2026-06-30 — Department match must be by dept ID CODE, not name (name tiers miss abbreviation drift)

**Tried:** `pickDepartmentOptionIndex` matched only on the department NAME (Tier 1 desc⊆option, Tier 2 option-name⊆desc). `fillFinalTransactions` was called with `department: jobSummaryData.departmentDescription` only — the `deptId` UCPath already extracts was never passed.
**Failed because:** UCPath Job Summary's department DESCRIPTION uses abbreviations the Kuali option spells out. Live (docs 4403/4406/4407/4408, ECEC separations): UCPath `"EARLY CHILDHOOD EDU. CENTER"` (deptId `000141`) never matched Kuali `"000141 - Early Childhood Education Center"` — Tier 1 fails (`"edu. center"` ⊄ `"education center"`) and Tier 2 fails (`"early childhood education center"` ⊄ `"early childhood edu. center"`). Result: `best match: "NONE"`, Department dropdown left on `"- - -"`, every ECEC doc shipped a blank department.
