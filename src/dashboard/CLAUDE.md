...
- **2026-05-27: Input-bar row shape is count-based.** A dashboard input run with one parsed value should render one flat row; a run with multiple parsed values should render one batch row, even for utility workflows such as `eid-lookup` that use passive children when delegated by OCR.
