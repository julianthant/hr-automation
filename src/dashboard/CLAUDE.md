...
| `/api/cancel-queued` | POST | Body: `{workflow, id, runId?, scope?}`. Cancels the SQLite task/attempt, writes completed `worker_commands.cancel_task`, emits JSONL audit + tracker `failed` with `step:"cancelled"`. 409 if claimed. | `QueueItemControls` via `useWorkflowActionDispatcher` |
...
| `/api/task/force-stop` | POST | Body: `{workflow, id, runId?, scope?}`. Writes `force_stop_task` plus scoped `kill_browser` commands for browser rows attached to that task. | `CancelRunningButton` via `useWorkflowActionDispatcher` |
