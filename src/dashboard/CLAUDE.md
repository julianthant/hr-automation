...
| `/api/sharepoint-download/run` | POST | `{ok, path, filename}` or `{ok:false, error}` — launches headed SharePoint download via `buildSharePointRosterDownloadHandler` (in `src/workflows/sharepoint-download/`), saves to `src/data/` | `QueuePanel` Download button (always visible, all workflows) |
...
- **2026-04-22: Roster download button in queue header.** The `QueuePanel` header surfaces a Download icon (lucide-react `Download` / `Loader2`) to the right of the search input for every workflow. Click fires `POST /api/sharepoint-download/run`, whose handler (`buildSharePointRosterDownloadHandler`) lives in `src/workflows/sharepoint-download/handler.ts` — `dashboard.ts` just wires it into the HTTP route. The button is intentionally global: operators often pull the latest onboarding spreadsheet while looking at any workflow's queue. Handler reads `ONBOARDING_ROSTER_URL` from env (missing → 400), holds a module-level in-flight lock (concurrent → 409), and invokes `downloadSharePointFile` (`src/workflows/sharepoint-download/download.ts`) to save into `src/data/`. Frontend toasts: `loading` on click, `success` with saved path, `warning` on 409, `error` on 4xx/5xx. The handler + helper live in a dedicated non-kernel workflow directory (no `defineWorkflow`, no tracker JSONL) so it stays out of the TopBar dropdown — see `src/workflows/sharepoint-download/CLAUDE.md`. Deliberately does **not** reintroduce the 2026-04-18-removed runner / child-process registry — this is a single in-process helper call on the SSE server. Endpoint was renamed from `/api/emergency-contact/download-roster` to `/api/sharepoint-download/run` in the same commit; there are no remaining references to the old path.
