...
| `/events?workflow=X&date=Y` | SSE | `{entries, workflows, wfCounts, failureCounts}` enriched + `stepDurations` every 1s. `failureCounts` is `Record<workflow, number>` of failed entries on the active date (latest run per id). | `useEntries` hook (drives `FailureBell` badge) |
...
| `/events/sessions` | SSE | `SessionState` (workflow instances + browsers + duo queue) | `useSessions` → `TerminalDrawer` |
