...
- **2026-05-20: Queue rows render from workflow runtime projections.** `QueuePanel` now builds `WorkflowRunProjection` rows from the existing queue-surface classifier and passes projection titles, subtitles, row labels, batch members, and action descriptors into `EntryItem`, delegation/batch group cards, batch member lists, and the log-panel row-type chip. Batch footer retry/delete targets must come from projection action `targetRunIds`; batch-view bulk actions stay scoped to members whose `parentRunId` matches the opened projection and must not reach hidden rows outside that batch.
