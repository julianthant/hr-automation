...
The backend entrypoint is `src/tracker/dashboard.ts`, which now delegates HTTP routing to modular Hono routes under `src/tracker/dashboard/hono/routes/`. The Vite dev server (port 5173) proxies `/api/*` and `/events/*` to it via `vite.dashboard.config.ts`; `dashboard --prod` serves `dist/dashboard/index.html` from the same Hono server.
...
| `/api/delete-bulk` | POST | Body: `{workflow, date, ids?[], items?:[{id, runId?}]}` — delete many tracker rows + scoped screenshots. | `DeleteAllButton` |
...
| `/api/daemons` | GET | `DaemonInfo[]` — SQLite workers plus lockfile fallback, heartbeat age, current item/run, and scoped browser processes. | `useDaemons` → `TerminalDrawer` / `DaemonRow` |
...
| `/api/preview-inbox` | GET | `PreviewInboxRow[]` — cross-workflow preview rows (`data.mode === "prepare"`, status `done`, step ≠ `approved`/`discarded`) sorted newest first | _(removed 2026-05-06)_ |
...
| `/events/hub?subs=<encoded JSON>` | SSE | `{sub, data, event?}` envelopes per subscription | All SSE hooks via `sseHub.subscribe(topic, params, listener)` (see `src/tracker/dashboard/hono/topics.ts` for topic registry) |
...
- Tracker JSONL + screenshots older than **30 days** (filename date / embedded timestamp) are pruned on dashboard startup and on each `GET /api/preflight` — same threshold as `startDashboard()` (`cleanMaxAgeDays`, default 30). **`npm run clean:tracker`** remains a separate CLI with default **7** days if you want a manual, stricter prune.
...
- **2026-05-14: Prep-mode rows are always batch anchors.** `queue-surface-classifier` now classifies any tracker row with `data.mode === "prepare"` as the persistent batch parent (predicate: `isPrepBatchAnchor`), regardless of approval state and even when the batch has zero or one child. The batch surface toolbar title resolves to `parent.data.__name` (e.g. `Oath Signature · #1234`) via `resolveDaemonBatchQueueTitle`'s new optional `titleOverride` parameter, threaded from `QueuePanel.tsx`'s `<BatchQueueToolbar titleOverride={resolvedBatchToolbarEntry.data?.__name} />` call-site.
...
- **2026-04-14 / 2026-05-05: Session drawer empty states** — The session surface should always render; show "No active workflows" / "No daemon workers" placeholders when empty. User preference is stable layout over auto-collapse for the bottom session drawer.
...
src/dashboard/
  ...
  components/
    ...
    TerminalDrawer.tsx, DaemonsSection.tsx, DaemonRow.tsx, WorkflowBox.tsx, BrowserChip.tsx, SelectorWarningsPanel.tsx
