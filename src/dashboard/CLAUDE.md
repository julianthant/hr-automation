...
- **2026-05-22: Approved single-signer prep rows must stay batch cards.** `buildTrackerQueueSurfaces` (`src/tracker/queue-surfaces.ts`) collapsed an *approved* prep batch-parent that had exactly one delegated member into a flat single row (`singleDelegationEntries`). Before OCR approval `approved` is `false`, so a single-signer PDF rendered as an `approval-delegation` card; after approval it turned into a plain row — the row type changed across the approval boundary. Removed the `approved && members.length === 1` collapse: an approved prep batch-parent now always emits an `approval-delegation` card regardless of signer count. The `members.length === 0` branch stays — an approved prep row with no *visible* members (e.g. the OCR tab showing a prep row whose signer children are oath-signature rows in another workflow's queue) still renders flat. Rule: a prep/upload batch-parent is one operator upload action; its surface kind is stable for the whole lifecycle. Regression coverage: `tests/unit/dashboard/queue-surface-classifier-prep-batch.test.ts` (`post-approval prep parent + single kernel child stays grouped`).
- **2026-05-22: OCR key pool popover filters by provider.** `ApiLimitsIndicator` keeps the TopBar trigger compact but the popover header now uses a provider dropdown, defaulting to Gemini, instead of an overall health badge and pool-wide request/reset summary. Keep provider-specific inspection local to the selected provider so the list stays short as fallback OCR providers are added. No component-level tests currently cover this indicator; verify with lint, typecheck, and dashboard build until a dashboard component harness exists.
...
- **2026-05-21: Multi-file upload is N independent single-file runs (no grouping card).** `RunModal` still lets the operator select multiple PDFs and posts each one separately to its workflow's submit URL, but the shared `originBatchRunId` / `originBatchSubject` grouping was removed (`buildBatchSubject` deleted). Each PDF becomes its own top-level prep row/card titled by PDF name — uploading N PDFs at once is identical to uploading them one at a time. Backend: `/api/ocr/prepare` no longer parses/uses those fields, and `writeOriginParentPending` emits prep rows with no `parentRunId`. Operator request: treat each PDF as a separate single run, not one task bundling multiple PDFs.
