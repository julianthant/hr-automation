...
- **2026-05-27: Queue surfaces classify shape × scope.** `buildTrackerQueueSurfaces` reads canonical row shape (`single` / `batch` / `batch-member`) plus `parentRunId` scope; delegated child presentation comes from the parent workflow policy. OCR declares `delegation.flatMemberChildWorkflows` for `eid-lookup` and `active-check`, so those children stay flat `delegation-member` rows instead of being promoted to a batch group.
- **2026-05-27: Input-bar row shape is count-based.** A dashboard input run with one parsed value should render one flat row; a run with multiple parsed values should render one batch row, even for utility workflows such as `eid-lookup` that render as flat members when delegated by OCR.
