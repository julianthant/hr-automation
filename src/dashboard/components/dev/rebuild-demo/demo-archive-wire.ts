...
export const DEMO_CHANGE_RECORDS: ChangeRecordWire[] = [
  ...
  /**
   * A MINOR app bump, and it is in the corpus on purpose: it is the record that
   * makes the new two-part app scheme legible. It touched no workflow, so it
   * archived nothing and nothing could have blocked it — the whole distinction
   * the date stamp could not express.
   */
  {
    id: "chg-app-3-1",
    scope: "dashboard",
    ...
    at: "Jul 24, 8:35 AM",
    by: DEMO_OPERATOR,
    what: "Queue chips, the toolbar and the report header were re-cut; the rail now counts active over total.",
    why: "Presentation only — no workflow's steps, data contract or projection moved, so no run's shape changed and nothing archived.",
    commit: "c41d9e0",
    fromVersion: "app 3.0",
    toVersion: "app 3.1",
    archivedRuns: 0,
  },
  {
    ...
    // 2.4 → 3.0 —", which is exactly what the version column beside it already
    // prints. A description that restates the chip next to it is the same fact
    // twice, and it is the copy that gets truncated.
...
const BULK_SHAPES: {
  ...
  { workflowId: "separations", version: 6, minor: 0, bumpId: "chg-app-3-0", statuses: ["verifiedDone", "verifiedDone", "doneWarnings", "cancelled"] },
  { workflowId: "onboarding", version: 4, minor: 2, bumpId: "chg-app-3-0", statuses: ["verifiedDone", "verifiedDone", "verifiedDone", "failed"] },
  { workflowId: "work-study", version: 2, minor: 0, bumpId: "chg-app-3-0", statuses: ["verifiedDone", "doneWarnings"] },
  { workflowId: "ocr", version: 8, minor: 0, bumpId: "chg-ocr-9", statuses: ["verifiedDone", "verifiedDone", "doneWarnings"] },
  { workflowId: "oath-signature", version: 5, minor: 1, bumpId: "chg-app-3-0", statuses: ["verifiedDone", "verifiedDone", "cancelled"] },
  { workflowId: "person-lookup", version: 3, minor: 0, bumpId: "chg-app-3-0", statuses: ["verifiedDone", "verifiedDone", "verifiedDone"] },
  { workflowId: "i9-check", version: 1, minor: 0, bumpId: "chg-app-3-0", statuses: ["verifiedDone", "doneWarnings", "failed"] },
];
