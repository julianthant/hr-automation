...
export function buildSyntheticBatchQueueAnchor(
  ...
  return {
    ...
    data: {
      pdfOriginalName:
        members.length > 0 ? `${label} batch (${members.length})` : `${label} batch`,
    },
  };
}
 ...
 * this component is the integration point for future batch-scoped chrome
 * (e.g. daemon batch metadata) without changing the main {@link QueuePanel} list.
 */
