...
| `queue-panel/` | Left queue column and queue row controls/status helpers. | `QueuePanel.tsx`, `EntryItem.tsx`, `StatPills.tsx`, `QueueItemControls.tsx`, `queue-status.ts` |
...
| `ocr/` | OCR/prep review UI, parent/child rows, OCR record renderers, and OCR-only types/helpers. | `OcrReviewPane.tsx`, `OcrQueueRow.tsx`, `record-renderers.tsx`, `preview-gate.ts` |
