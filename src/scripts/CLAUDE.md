...
- `clean-tracker.ts` — Prunes `.tracker/*.jsonl`, `.screenshots/*.png`, AND `.tracker/step-cache/**` files older than N days. Flags: `--days N`, `--dir`, `--screenshots-dir`, `--step-cache-dir`, `--no-screenshots`, `--no-step-cache`, `--screenshots-only`, `--step-cache-only`. Default behavior cleans all three. Wired as `npm run clean:tracker`. Exports `cleanTrackerMain` for testability; return shape is `{ trackerDeleted, screenshotsDeleted, stepCacheDeleted }`.
