...
- **`duo-webauthn-enroll.ts`** — One-time enrollment of a CDP virtual authenticator as an independent Duo security key for hands-off Duo MFA. Signs into `duo.ucsd.edu`, completes the first Duo with a single phone approval (the script **auto-sends a Duo Push** for that first prompt via a fire-once `recovery` on its `pollDuoApproval` — the device-portal prompt defaults to a platform-auth screen whose native dialog hides the factor text, so the poll loop's own factor auto-push never matches; without this the first Duo silently times out with nothing on the phone), drives "Add a device → Touch ID" against a virtual authenticator, and **merges** the created credential into `.auth/duo-webauthn.json` (gitignored) keyed by transport — so enrolling a second device preserves the first. Flags: `--security-key` (enroll a `usb` device instead of `internal`/Touch ID; auto-injects the CTAP2 `isExternalCTAP2SecurityKeySupported` shim so Duo offers the "Add Security key" tile in Playwright Chromium), `--name "<label>"`, `--out <path>`. Exports `parseEnrollArgs` + `enrollDuoWebAuthn` for tests. Wired as `npm run duo:webauthn:enroll`. Runtime replay (multi-credential, one authenticator per transport) + the `HR_AUTOMATION_DUO_WEBAUTHN=1` flag live in `src/infra/auth/duo-webauthn.ts` (see `src/infra/auth/CLAUDE.md`).
- **`setup.ts`** — First-use environment validation wizard. Fixed checks for `.env` keys (existence only, never values), Node ≥ 26, `tsx`, Playwright chromium cache, `.tracker/` + `.screenshots/` + `~/Documents/onboarding/` writability, macOS notification capability (warn-only on non-darwin), optional `jq`. Prints `[ok]` / `[warn]` / `[fail]` per check with a fix suggestion. Exits 0 if all pass or only warnings; exits 1 on any failure. Wired as `npm run setup`. Exports `runAllChecks`, `setupMain`, `writeEnvVar` for tests.
