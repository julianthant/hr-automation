...
- **`duo-webauthn-enroll.ts`** — One-time enrollment of a CDP virtual authenticator as an independent Duo security key for hands-off Duo MFA. Signs into `duo.ucsd.edu`, completes the first Duo manually (one phone approval), drives "Add a device → Touch ID" against a virtual authenticator, and **merges** the created credential into `.auth/duo-webauthn.json` (gitignored) keyed by transport — so enrolling a second device preserves the first. Flags: `--security-key` (enroll a `usb` device instead of `internal`/Touch ID; auto-injects the CTAP2 `isExternalCTAP2SecurityKeySupported` shim so Duo offers the "Add Security key" tile in Playwright Chromium), `--name "<label>"`, `--out <path>`. Exports `parseEnrollArgs` + `enrollDuoWebAuthn` for tests. Wired as `npm run duo:webauthn:enroll`. Runtime replay (multi-credential, one authenticator per transport) + the `HR_AUTOMATION_DUO_WEBAUTHN=1` flag live in `src/infra/auth/duo-webauthn.ts` (see `src/infra/auth/CLAUDE.md`).
