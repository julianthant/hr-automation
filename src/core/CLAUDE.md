...
- **2026-06-02: Root trace-id propagation — the FULL root trace id (not just the code) now propagates transitively to every descendant.** A parallel channel to `rootCode`: `rootTraceId` carries the originating run's complete `<code>-<HHMMSS>-<runId4>` id so every row of one operation DISPLAYS the identical id while each keeps its own runId/itemId (logs/SQLite/footer `#run` untouched). Plumbing mirrors `rootCode`: `delegate.ts` (`DelegateCoreArgs.rootTraceId` → `withRootRuntimeOptions` merges it onto `__runtimeOptions`; pre-emit sites pass it as `buildPendingTrackerData`'s verbatim `traceId` opt) → SQLite task store → `normalizeRuntimeOptions` (`rootTraceId`) → BOTH seed paths in lockstep: `findFrozenTraceId(...) ?? runtimeOptions?.rootTraceId ?? buildTraceId(...)` (frozen-once stays FIRST). **Transitivity (unlike `rootCode`):** `makeCtx` computes `forwardRootTraceId = parent.rootTraceId ?? findFrozenTraceId(parent) ?? undefined` once and threads it into `buildDelegateApi`, so a root reads its OWN frozen id and forwards it, while a non-root passes through what it inherited → grandchildren show the ORIGINAL root (the `parentSubject` pattern). **Gotcha:** a physical root's `forwardRootTraceId` auto-read only works once a row carries the id — a non-preAssigned `runWorkflow` root has a pending row (seeded), but the real OCR root passes `rootTraceId` EXPLICITLY at its fan-out sites (orchestrator's person-lookup `delegateToAllImpl`, and approve.ts's HTTP fan-out which reads the OCR row's frozen id via `findFrozenTraceId`), not relying on the auto-read. OCR brands the operation via the form spec's `traceCode` (oath → `ou`; standalone/EC keep `oc`). Cancel/shutdown needs NO change — `preserveFrozenTraceId` already copies the frozen id, which now carries the root id. Pinned by `ctx-delegate-to.test.ts` (verbatim + 3-level transitivity), `run-one-item.test.ts` + `run-workflow-kind-trace.test.ts` (both seed paths), `daemon/shutdown.test.ts`, `ocr-approve-oath-fanout.test.ts`, and `tests/scenarios/delegate-to/oath-upload-shape.test.ts`.
