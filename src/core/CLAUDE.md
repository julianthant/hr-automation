...
- **`data.archetype` is the canonical row-type discriminator.** The kernel stamps it on every per-item row; orchestrator/prep code must stamp it explicitly on `batch-parent`/`dispatch` rows. Dashboard queue surface, log panel footer chip, and display-name resolver all dispatch on this one field. New writes must stamp `data.archetype` — declare `archetype` on every `defineWorkflow` call (architecture guard: `tests/unit/architecture/archetype-coverage.test.ts`). `resolveRowArchetype` has read-time fallbacks for legacy rows.
