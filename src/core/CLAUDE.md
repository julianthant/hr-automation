...
- **Daemon spawn is serialized per workflow.** `ensureDaemonsAndEnqueue` wraps daemon discover + spawn in `withDaemonSpawnLock` (in-process, keyed by `workflow` + `trackerDir`). Without it the discover→spawn window is a TOCTOU race: two near-simultaneous enqueues for one workflow — rapid OCR approvals, a double dashboard click — both observe 0 alive daemons and each spawn one, producing duplicate `<Workflow> 1 / <Workflow> 2` daemons. The second caller waits, re-discovers (the alive-daemons TTL cache is invalidated inside the lock so the predecessor's just-spawned daemon is visible), and reuses it — `computeSpawnPlan` returns 0. The lock is in-process only; concurrent enqueues from separate CLI processes are not covered.
