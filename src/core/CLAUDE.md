...
- **2026-05-31: `queueRowKind` + `__traceId` must ride EVERY row, not just `pending`.** They were stamped only by `buildPendingTrackerData` (the pre-emit path); live rows (`running`/`done`/`failed` + the synthetic auth-timing rows) build their `data` from `stringifiedSeed` → `withTrackedWorkflow`, which carries `__subject`/`archetype` but not these two. Because the dashboard collapses a run to its *latest* row (`dedupeLatestByIdWithCarriedEmplId`), the kind/trace-id vanished the instant the run left `pending`, and the kind-driven title/subtitle fell back. Fix in `run-one-item.ts`: seed `queueRowKind` (recomputed via `resolveQueueRowKindFromValue`) and `__traceId` onto `stringifiedSeed` so they propagate to every emission. The trace id is **frozen once** — read back from the run's first pending row via `findFrozenTraceId` (shared helper in `find-latest-entry.ts`), falling back to a fresh `buildTraceId` only when no prior row exists (in-process `runWorkflow`). The worker's own `pending` re-emit passes that frozen id to `buildPendingTrackerData`'s new `traceId` opt so it doesn't recompute a time-drifted one. Pinned by `tests/unit/tracker/find-frozen-trace-id.test.ts`, `pending-data-root-code.test.ts`, and the scenario snapshots (which now show both fields on terminal rows).
