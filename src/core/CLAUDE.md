...
- **Cancel is one mechanism via per-run `AbortController` + unified `RunRegistry` (Contract 5).** Every `runOneItem` / `runWorkflow` invocation constructs an `AbortController`, registers a `RunHandle` (with the controller + the `Session`) on the module-level `runRegistry` (`src/core/run-registry.ts`), and unregisters in `finally`. The controller's `signal` rides on `ctx.signal` and is auto-injected into every Playwright method that accepts `signal?: AbortSignal` via the Page proxy returned by `ctx.page(id)` (see `src/core/kernel/page-proxy.ts`). Every cancel trigger — daemon `cancel_task` worker command, HTTP `/cancel-current`, browser-disconnect handler, dashboard in-process cancel route, daemon shutdown sweep — calls `runRegistry.cancel(runId, { reason })`. `cancel` aborts the controller (any in-flight Playwright call rejects within ms), writes the SQLite cancel audit when the handle carries `control` metadata (in-process runs), and schedules a watchdog that hard-kills chromium after `hardKillAfterMs` (default 5000ms; 0 disables, used by daemon shutdown) only if the run hasn't unregistered by then — covers pre-handler launch hangs (e.g. stuck on Duo). Daemon state collapses the pre-Phase-1 `cancelTarget` / `currentRunController` / `inFlight` triple into a single `state.activeRun: RunHandle | null`. The stepper's between-step probe reads `controller.signal.aborted` as the synchronous-checkpoint when no Playwright call is in flight; both paths converge on `CancelledError('cancelled')` + `step: "cancelled"` on the terminal row. Soft/force-stop distinction is gone — there is no `/api/task/force-stop` route, no `CancelMode` type, no Force Stop button. Architecture guard: `tests/unit/architecture/cancel-mechanism.test.ts` blocks reintroduction of the legacy two-runtime split. See `src/control/CLAUDE.md` → "Cancel contract".
...
- **Browser disconnect during shutdown is cancellation.** Route through `requestCancel`/`runRegistry.cancel(state.activeRun.runId)` before waking shutdown so the per-run `AbortController` aborts and `Stepper.step` reclassifies browser/page-closed errors as `CancelledError` instead of failed workflow work.
