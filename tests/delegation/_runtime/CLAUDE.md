...
| `rt.daemons(wf)` | Liveness-probed alive daemons `{ port, instanceId }[]` (via `findAliveDaemons`, cache-invalidated). Count the pool before/after a teardown. |
| `rt.wake(wf)` | POST `/wake` to every alive daemon of `wf` (the idle-wake path). |
| `rt.stopAll(wf, { force? })` | Workflow-scoped **Stop All** — POST `/stop` (NO `reassign`) to every alive daemon. In-flight items FAIL red (no `step:"cancelled"`), queued items terminalized by the last daemon. The teardown-soak stop-all leg. |
| `rt.stopInstance(wf, { reassign?, holdingRunId? })` | Per-instance card stop — POST `/stop` `{ reassign: true }` to ONE daemon. With `holdingRunId`, targets the daemon whose `/status.inFlightRunId` matches (so reassign actually has an item to bounce); else `alive[0]`. Returns the stopped `{ instanceId, port }`. |
