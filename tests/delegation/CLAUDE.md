...
for the lone-member case. EC has NO `traceCode` (E2E-007, reverses F5), so a standalone EC OCR upload brands the default `oc-` prefix; every child shares that `oc-…` prefix (oath brands `ou-`).
