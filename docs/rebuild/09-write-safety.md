...
| **④ Fence** | Check `probeToFenceMaxMs` **and** that `WriteBindingProof` was created after prepare for this run/attempt/lease. INSERT the unseen key or CAS its `retryable` row to `attempting`, incrementing generation. Store the parsed binding proof and enqueue `write.attempting` in the same SQLite transaction. The permanent primary key is the same-key mutex. | durable intent + binding proof + span outbox | elapsed proof or binding mismatch ⇒ no click; CAS conflict ⇒ discard staged page/no click; no generic retry after a won fence; a transient storage failure (`SQLITE_BUSY`) **throws** — nothing external has happened yet, so failing closed costs nothing (D94) |
...
| **⑦ Atomic durable commit** | ONE SQLite transaction marks the intent committed and writes the schema-valid `TransactionOutcome` checkpoint, immutable ledger outbox row (including binding-proof digest), terminal-span outbox row, evidence receipt ref, and run state. JSONL ledger/span projectors run afterward. | all authorities/outboxes durable together | a transient storage failure (`SQLITE_BUSY`) **retries** with bounded backoff — the HR mutation is already real, so throwing here would discard proof of a write that exists (D94); exhausted retries escalate to a durable out-of-band record + critical notification and leave the intent `attempting` for recovery, never reporting the write as absent or lost; any other transaction failure leaves intent attempting and enters recovery; projector lag is repairable and never changes write outcome |
...
**Retry asymmetry across beat ⑤ (D94).** Beat ⑤ is the point of no return, so a *transient storage*
failure means opposite things on either side of it and must not be handled uniformly. Before it
(①–④), throw: nothing external happened, and failing closed is free. After it (⑦), retry: the HR
mutation already exists, and throwing would discard its proof — manufacturing precisely the
unknown-outcome ambiguity this document exists to eliminate. The dangerous implementation is the
*natural* one — wrapping every authority write in the same handler — which is why the asymmetry is a
named decision with a guard rather than a code-review convention. Retrying the identical commit is
not a silent fallback under the fail-loud rule: it re-runs one operation, it never substitutes a
value. Note the beat-⑦ retry is bounded by the fact that no `await` may sit inside the transaction
(D95), so a retry contends for milliseconds, not for the length of a page interaction.
