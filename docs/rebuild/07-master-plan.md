...
| Data-service systems: CSV/PDF extraction + roster matching stores, operator column mapping, Edit Data checkpoint UI | **Doc 06 (PENDING)** |
...
(reconciliation, both rounds), 05 (execution), 08 (gap audit), 09 (write-safety), 10 (guards), 11
(clock/config/secrets) are **written and reconciled**. Three adversarial review rounds are folded
into D1–D22.

**What remains (the two operator sign-offs).**
1. **Doc 06 — data-intake & Edit Data.** Not yet written. Scope from charter §11 (first-class
   `extraction`/`ocr`/`roster` service systems + **operator-defined column mapping** → workflow zod
   input) + §12 (Edit Data over checkpoints — checkpoint state always live-visible + editable,
   schema-validated on save) + gap-audit `08`. Must specify: the column-mapping model
...
5. **Delete old code** for any system now fully migrated; update the nearest CLAUDE.md + owning docs.
...
| 3 | **ocr** pipeline + **extraction**/**roster** service stores | service | No browser, no submit; unblocks every OCR/roster-dependent workflow below (build here, before its consumers) |
...
5. **Freshness maxAges** — for each read whose output feeds a write, the `freshness.maxAgeMs` (D8);
...
- **Column mapping + Edit-Data UI** (doc 06) — a **dashboard** concern. It lands **when the scoped
  ...
  migration that needs mapping (i9-check order 5 / onboarding order 9) and the first parked run that
  needs Edit Data. It is NOT on the base critical path; until it flips, ingest uses the ported
  path and Edit Data proxies to the old endpoint (D13).
...
- **Scoped dashboard flip (doc 03 §5.4 / D13).** The flip is **not wholesale** (review 03 sized that
  ...
  **one-week legacy-dashboard fallback** stays after the flip.
