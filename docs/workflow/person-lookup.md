...
The workflow treats UCPath Last Hire as the employee `startDate` and the assignment row EFFDT as backend `effdt` context. CRM date cross-verification compares CRM First Day of Service to the UCPath start date when available, falling back to EFFDT only when Last Hire is missing. Dashboard detail fields show `Start Date`, not EFFDT.
...
- One OCR person creates a one-member delegated batch surface in the Person Lookup tab.
- Multiple OCR people with the same OCR parent form the same kind of batch surface.
...
| OCR utility child | Delegated batch surface, even when there is only one lookup. | Person/EID, preferring resolved person/EID over technical OCR retry ids. | Normal child footer. | Appears in the Person Lookup workflow tab. | Cancel/retry affects only that lookup/person. |
