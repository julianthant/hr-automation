...
npm run dashboard:prod       # Serve pre-built dashboard from the Hono dashboard server
...
Dupe-protection: the kernel provides no tracker-side idempotency cache or step-cache (both primitives removed 2026-04-23). Workflows that need to avoid double-submits must use live-page probes — e.g. separations' `ucpath-transaction` calls `findExistingTerminationTransaction(page, employeeId, effectiveDate)` (from `src/systems/ucpath/transaction.ts`) to scan the Smart HR Transactions list for a row whose Person ID cell matches this EID and row text includes the effective date + "Terminatn" action before submitting; oath-signature checks the "no oath signature" sentinel on the loaded profile. EID match beats name match because Kuali↔UCPath name variants (nicknames, spelling, column-order errors) silently miss dupes — Aki Uchida (EID 10794813) hit that on 2026-04-24. Read-only extraction work (e.g. onboarding's CRM `extraction` step) is re-run on every retry — cheap relative to the correctness cost of serving a stale cached extraction when the user fixed bad data between runs.
...
Per-system gotchas live in `src/systems/<system>/AGENTS.md`. Per-workflow gotchas live in `src/workflows/<name>/AGENTS.md`.
...
- **`src/systems/<sys>/SELECTORS.md`** — auto-generated catalog of every selector this system exports. Each entry has the FQN (e.g. `smartHR.tab.personalData`), one-line summary from JSDoc, `@tags`, and a clickable line ref into `selectors.ts`. Regenerate after any selectors.ts change with `npm run selectors:catalog`. Committed so future Codex sessions see the catalog without running anything. A unit test (`tests/unit/scripts/selectors-catalog.test.ts`) gates drift — PRs that change selectors without regenerating fail there.
...
- **`src/systems/<sys>/common-intents.txt`** — hand-curated 5-10 typical intents per system. Useful reference when authoring a new workflow's AGENTS.md `selector:search` examples.
...
Each per-system `AGENTS.md` links to its `LESSONS.md` + `SELECTORS.md` and embeds this loop verbatim.
...
- **Step-cache + idempotency primitives removed (2026-04-23).** `src/core/step-cache.ts` and `src/core/idempotency.ts` were deleted along with all callers. Step-cache caused stale-data retries (correctly-fixed Kuali data was overridden by cached extractions); idempotency's tracker-side cache added complexity without preventing the failure modes it was designed to catch. Separations now uses a pre-submit existence check against the Smart HR Transactions list (`findExistingTerminationTransaction`); onboarding + work-study have no dupe-protection beyond the handler's own ordering; oath-signature retains only its live-page probe. Kernel-level resume stays deferred.
...
After every error fix, selector re-map, or new pattern: update the relevant AGENTS.md. These files are the only memory between sessions — keep them accurate. Add notes to `## Lessons Learned` in the module/workflow you touched; bump `// verified` dates in `selectors.ts` when you re-map a selector; keep gotchas current.

## Codex-mem — reflexive memory search

Codex-mem ([thedotmack/Codex-mem](https://github.com/thedotmack/Codex-mem)) auto-captures observations from every session into a SQLite + Chroma store, but does NOT auto-query. Before any non-trivial task — planning, refactoring, implementing a new workflow/selector/system, debugging a recurring or non-obvious issue, or answering "have we seen this before" / "how did we handle X" / "what's the status of Y" — query memory as one of your first actions. Skip only for trivial one-liners, reading a single known file, or unambiguous step-by-step instructions. If the query returns nothing useful, proceed normally — don't stall.
...
The `Codex-mem:mem-search` skill wraps this loop — invoke it via the Skill tool instead of orchestrating layers by hand for one-off queries.
...
- `Codex-mem:mem-search` — natural-language query over observations (wraps the 3-layer loop)
- `Codex-mem:knowledge-agent` — build / query a focused corpus
- `Codex-mem:smart-explore` — tree-sitter-based structural code search (alternative to reading whole files when you need shape, not content)
- `Codex-mem:make-plan` / `Codex-mem:do` — phased plan creation + subagent execution
- `Codex-mem:timeline-report` — narrative project-history report
...
# [hr-automation] recent context, 2026-05-04 10:20pm PDT
